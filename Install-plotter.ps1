...
function get_bladebit_url()
{
    ...
    $GITHUB_BASE_URL = "https://github.com/HDDcoin-Network/bladebit/releases/download"
...
function get_madmax_filename()
{
    ...
    $hddcoin_plot = "hddcoin_plot"
    if ("${ksize}" -eq "k34")
    {
        $hddcoin_plot = "hddcoin_plot_k34"
    }
    $suffix = ""
    if ("${os}" -eq "macos")
    {
...
function get_madmax_url()
{
    ...
    $GITHUB_BASE_URL = "https://github.com/HDDcoin-Network/hddcoin-plotter-madmax/releases/download"
...
try {
    ...
    elseif("${plotter}" -eq "madmax")
    {
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "hddcoin_plot.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax to $(Get-Location)\hddcoin_plot.exe"
        }
        catch {
            Write-Output "ERROR: Download failed. Maybe specified version of the binary does not exist."
            Pop-Location
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "hddcoin_plot_k34.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax for k34 to $(Get-Location)\hddcoin_plot_k34.exe"
        }
        catch {
