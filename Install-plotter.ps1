...
function Get-MadmaxUrl()
{
    ...
    $GITHUB_BASE_URL = "https://github.com/Chia-Network/hddcoin-plotter-madmax/releases/download"
...
try {
    ...
    elseif("${plotter}" -eq "madmax")
    {
        ...
        $dest_dir = $PWD
        Get-Binary -url $url -dest_dir $dest_dir -new_filename "hddcoin_plot_k32.exe"

        $url = Get-MadmaxUrl -ksize "k34" -ver $version -os $os -arch $arch
        $dest_dir = $PWD
        Get-Binary -url $url -dest_dir $dest_dir -new_filename "hddcoin_plot_k34.exe"
    }
    else
    {
