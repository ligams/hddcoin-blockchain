...
get_madmax_filename() {
  ...
  export HDDCOIN_PLOT="hddcoin_plot"
  if [ "$KSIZE" = "k34" ]; then
    export HDDCOIN_PLOT="hddcoin_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
...
# Handle MadMax binaries
  ...
  handle_binary "$url" "$madmax_filename" "hddcoin_plot"
  # k34 MadMax binary
  url="$(get_madmax_url "k34" "$VERSION" "$OS" "$ARCH")"
  madmax_filename="$(get_madmax_filename "k34" "$VERSION" "$OS" "$ARCH")"
  handle_binary "$url" "$madmax_filename" "hddcoin_plot_k34"
else
  usage
fi
