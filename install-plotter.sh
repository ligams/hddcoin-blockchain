...
get_madmax_filename() {
  ...
  echo "${HDDCOIN_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" hddcoin_plot
  chmod 755 hddcoin_plot
  echo "Successfully installed madmax to $(pwd)/hddcoin_plot"
  ...
  mv -f "${madmax_filename}" hddcoin_plot_k34
  chmod 755 hddcoin_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/hddcoin_plot_k34"
else
  usage
fi
