...
from hddcoin.simulator.block_tools import make_unfinished_block
...
async def run_sync_test(
    ...
    with tempfile.TemporaryDirectory() as root_dir:
        ...
        hddcoin_init(root_path, should_check_keys=False, v1_db=(db_version == 1))
        ...
        try:
            full_node.set_server(cast(HDDcoinServer, FakeServer()))
            ...
            peer: WSHDDcoinConnection = cast(WSHDDcoinConnection, FakePeer())
...
async def run_sync_checkpoint(
    ...
    hddcoin_init(root_path, should_check_keys=False, v1_db=False)
    config = load_config(root_path, "config.yaml")
    ...
    async with full_node.manage():
        peer: WSHDDcoinConnection = FakePeer()  # type: ignore[assignment]
