...
def build_exclusion_list(mypy_failures: List[str]) -> List[str]:
    ...
    #     # hddcoin/cmds/wallet_funcs.py:1251: error: Incompatible types in assignment (expression has type "str", variable has type "int")  [assignment] # noqa
...
@main.command()
@click.option("--check-exclusions/--no-check-exclusions", show_default=True, envvar="HDDCOIN_MANAGE_MYPY_CHECK_EXCLUSIONS")
def build_mypy_ini(check_exclusions: bool = False) -> None:
    ...
    mypy_config_data = (
        ...
        .replace("[mypy-hddcoin-exclusions]", exclusion_section)
    )
