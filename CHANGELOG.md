...
## 1.8.2 HDDcoin blockchain 2023-06-28
...
## 1.8.1 HDDcoin blockchain 2023-05-17
...
## 1.8.0 HDDcoin blockchain 2023-05-03
...
- Return fee estimates as integer bytes
...
## 1.7.1 HDDcoin blockchain 2023-03-22
...
## 1.7.0 HDDcoin blockchain 2023-02-15
...
- Use better check that we are on mainnet when deciding to use default HDDcoin DNS server
- Remove conflicting TXs before adding SpendBundle to Mempool in `add_spend_bundle`
- Try each HDDcoin DNS Server in list before trying introducers
...
- Offer security updates: Offers that are generated with this version cannot be accepted with older versions of HDDcoin - see blog post for details
...
## 1.6.2 HDDcoin blockchain 2023-01-03
...
- New GUI prefs location under `HDDCOIN_ROOT`
...
## 1.6.1 HDDcoin blockchain 2022-11-03
...
- HDDcoinvdf updated to 1.0.7
- Chiapos updated to 1.0.11
- Clvm_tools updated to 0.4.5
- HDDcoin_rs updated to 0.1.14
...
- Allow developers to easily use standard HDDcoin `clvm` puzzles and libraries
...
- Fixed issues where HDDcoinLisp was compiled during import requiring write access to the directory (#11257) (thanks @lourkeur). To force compilation, developers can set environment variable `HDDCOIN_DEV_COMPILE_CLVM_ON_IMPORT`
...
## 1.6.0 HDDcoin blockchain 2022-9-20
...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/HDDcoin-Network/hddcoin-blockchain-gui/issues/950)
...
## 1.5.1 HDDcoin blockchain 2022-8-23
...
- Substantially reduced overall size of HDDcoin packages
...
## 1.5.0 HDDcoin blockchain 2022-7-26
...
- Fixed [CVE-2022-36447] where in tokens previously minted on the HDDcoin blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the HDDcoin blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 HDDcoin blockchain 2022-6-29
...
- Updated warning about `HDDCOIN_ROOT` being set when running init
...
## 1.3.5 HDDcoin blockchain 2022-5-11
...
## 1.3.4 HDDcoin blockchain 2022-4-19
...
## 1.3.3 HDDcoin blockchain 2022-4-02
...
## 1.3.2 HDDcoin blockchain 2022-4-01

### Fixed

- Fixed OpenSSL vulnerability CVE-2022-0778

## 1.3.1 HDDcoin blockchain 2022-3-16
...
- Added checks to ensure wallet address prefixes are either `hdd` or `thdd`.
...
## 1.3.0 HDDcoin blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of HDDcoin (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores HDDcoin will not use when launching process pools. Using 0 will allow HDDcoin to use all cores for process pools. Set the default to 0 to allow HDDcoin to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
...
- When you close the HDDcoin app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the HDDcoin and relaunch the HDDcoin app.
