...
* Fix `hddcoin wallet coins list` by adding NFT, DID, DAO_CAT to wallets denominated in mojos
...
* Fix issues with upgrading Chia via RPM by claiming ownership of `/opt/hddcoin` in the RPM
* clean out `/opt/hddcoin` before install and after removal of rpm
...
* DID wallet coin_added by @ytx1991 in https://github.com/Chia-Network/hddcoin-blockchain/pull/16256
...
* Use BLS from `chia_rs` and stop using `blspy` wheel in hddcoin-blockchain
...
* Change `hddcoin show keys --show-mnemonic-seed` to also show farmer private key (thanks xchdata1)
...
- Simplify get_max_send_amount for HDD and CATs
- Added wallet id showing when using the 'hddcoin plotnft show' (thanks @d1m1trus)
...
- add dust warning message to hddcoin coins commands & cleanup code
- Fixed `hddcoin rpc status` output
...
- Fix `hddcoin farm summary` aborting early if no local full node present (fixes #16164) (thanks @xchdata1)
- fix typo in PendingTxCache
- rename `hddcoin data add_missing_files` `-f`/`--foldername` to `-d`/`--directory`
...
- Fix issue with GUI and `hddcoin plotters` on Linux when making compressed plots (Fixes #16187)
...
- Multi threaded `hddcoin plots check`
- Hard fork 2.0
...
- Support calling daemon commands from hddcoin rpc CLI
- Use latest blspy
...
- Cudaplot fix
- Fixed `hddcoin plotnft show` command
...
### Added
- Add `hddcoin wallet vcs` command for Verifiable Credential operations
- Add `hddcoin wallet clawback` command for clawback operations
- Add `hddcoin wallet did` commands `get_details`, `update_metadata`, `find_lost`, `message_spend`, `transfer`
...
- Improve help text for deprecated `hddcoin show` commands. Thanks (@notpeter)
- Have the simulator not rely on `hddcoin_root` as originally implemented.
- Fork infrastructure
- Update `--fee` options to indicate units are in HDD
...
### Fixed
- Fix `hddcoin wallet make_offer` short-option collision on `-r` (Fixes #14874) (Thanks @yyolk)
- Fix `GENERATOR_MOD2` to have the same cost as `GENERATOR_MOD`
- Subscribe to the change children of CATs
- Offer shows network's HDD like THDD on `testnet10` (Fixes #13393) (Thanks @yyolk)
...
### Added
- Added `hddcoin completion` command
...
- additional metrics for `state_changed` events (used by hddcoin-exporter)
- Python 3.11 support
- `hddcoin wallet check` CLI command
...
### Changed
- `hddcoin show -f` changed to output proper JSON
...
- Offer CLI command help now shows `--fee` units as HDD
- Optimized offer code by limiting `additions` recomputation
- `chia_rs` updated to 0.2.4
- Improve the help text for the `hddcoin peer` and `hddcoin peer -a` commands
...
- New `hddcoin wallet coins` CLI and RPCs for listing, splitting, and combining coins
- New on-chain notification for offers, specifically designed for NFT offers
- New full node dynamic fee estimator (`hddcoin show -f` and `get_fee_estimate` full node RPC)
...
- server: Fix invalid attribute accesses in `WSHDDcoinConnection`
...
- Add `hddcoin keys show --json` (display all keys as json) (thanks @kimsk)
- Add `hddcoin data get_sync_status` CLI and RPC.
...
- Add options `--max-coin-amount` and `--exclude-coin-ids` to `hddcoin wallet send`
- Add `--fingerprint` option to `hddcoin keys show`
- Add SECURITY.md
...
- Add GUI memo field when sending HDD
...
- Set GUI binary name to `hddcoin-blockchain` in the Fedora rpm and Ubuntu deb (fix #13847)
...
- Improve error message when sending more bytes than Spendable Balance
...
- Added CLI option `hddcoin keys label`
- Added REMARK to `ConditionOpcodes`
- Prevent creation of offers with 100% royalties
- Added `hddcoin peer` command to replace `hddcoin show -c`
...
- New wallet CLI option `hddcoin wallet sign_message`
...
- Removed "Total iterations since start" from `hddcoin show -s`
- Removed rate-limited wallet
- Removed the beta program link from the warning in the CLI
- Removed `--enable-data-server` from `hddcoin configure` CLI
...
- Dropped unused `hddcoin_minor_release_number`
...
- Improved logging for `hddcoin plotters version` errors
...
- Minter DID now shown in output of `hddcoin wallet nft get_info` and in GUI
...
- Deprecated `hddcoin plotters install` command
...
- Fixed hddcoin farm summary if using a remote full node (thanks @yan74)
...
- Fixed the run_block utility to use hddcoinlisp_deserialization
...
- Fixed `hddcoin show -s` with other options as well
...
- Show Usage when running `hddcoin plotters` (#13690)
...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/Chia-Network/hddcoin-blockchain-gui/issues/950)

### Known Issues

- The CLI command `hddcoin configure --enable-data-server`, and the `config.yaml` parameter at `data_layer.run_server` have no effect, and will be removed in the future
...
- Add `hddcoin db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the hddcoinlisp code when necessary
...
- Remove hddcoin.util.path.mkdir()
...
- Removing `logging.basicConfig()` from `HDDcoinServer.__init__()`
...
- Log `hddcoin_full_version_str` in daemon and all services
...
- Make sure hddcoin commands output help when no args are given (#11013) (Thanks @noneus!)
...
- Update `hddcoin wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
...
- Added `hddcoin wallet nft` command (see <https://docs.hddcoin.org/docs/13cli/did_cli>)
- Added `hddcoin wallet did` command (see <https://docs.hddcoin.org/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.hddcoin.org/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.hddcoin.org/docs/12rpcs/nft_rpcs>)
...
- Added CLI options `--sort-by-height` and –sort-by-relevance` to `hddcoin wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously hddcoin selected the largest coin
...
- Implemented `hddcoin rpc` command
...
- Added `-n`/`--new-address` option to `hddcoin wallet get_address`
...
- Fixed typo in command line argument parsing for `hddcoin db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `hddcoin configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `hddcoin db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `hddcoin show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `hddcoin wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `hddcoin wallet show`
...
- You cannot install and run hddcoin blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
- Fixed `hddcoin keys show` displaying non-observer-derived wallet address.
...
- Fixed exception when `hddcoin keys migrate` is run without needing migration.
...
- Added new CLI option, hddcoin keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using hddcoin keys derive search.
...
- CLI option hddcoin show will display the currently selected network (mainnet or testnet).
- CLI option hddcoin plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with hddcoin show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the hddcoin-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI hddcoin show -c when No Info should be displayed.
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/Chia-Network/hddcoin-blockchain/pull/9050).
...
- Switched IP address lookup to first use Chia's service ip.hddcoin.org.
...
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Chia-Network/hddcoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
- This is the 1.0 release of Chialisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [hddcoinlisp.com](https://hddcoinlisp.com) shortly.
...
- You should review the [release notes for v1.0.2](https://github.com/Chia-Network/hddcoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- Migration with CHIA_ROOT set does not crash hddcoin init
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Chia-Network/hddcoin-blockchain/issues/1189)
...
- We now hand build Apple Silicon native binary wheels for all hddcoin-blockchain dependencies and host them at [https://pypi.hddcoin.org/simple](https://pypi.hddcoin.org/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a hddcoinlisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during ChiaLisp native deserialization.
...
- Chia wheels not on Pypi and some dependecies not found there also are now on pypi.hddcoin.org.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Chia-Network/hddcoin-blockchain/releases/tag/1.0rc1) was well.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Chia Blockchain GUI](https://crowdin.com/project/hddcoin-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Chia-Network/hddcoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Chia-Network/hddcoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Chia-Network/hddcoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [hddcoin.org](https://hddcoin.org/). Among the improvements this gives the Chia blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- Testnet is now running at the primary port of 38444. Update your routers appropriately. This opens 8444 for mainnet.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Chia-Network/hddcoin-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/Chia-Network/hddcoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Chia-Network/hddcoin-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Chia-Network/hddcoin-blockchain/tree/dev/electron-react/src/locales) of the hddcoin-blockchain repository.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chia Network at backup.hddcoin.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chia backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.hddcoin.org, your own installation, or a third party's version of it.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Chia-Network/hddcoin-blockchain/wiki) and placed in [INSTALL](https://github.com/Chia-Network/hddcoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Chia-Network/hddcoin-blockchain/wiki/Quick-Start-Guide)
...
- Chia start commands like `hddcoin start farmer` and `hddcoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `hddcoin stop` is unlikely to work on Windows native for now. If `hddcoin start -r node` doesn't work you can force the run/ directory to be reset with `hddcoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Chia-Network/hddcoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Chia-Network/hddcoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- There is a new command `hddcoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `hddcoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/hddcoin-blockchain/wiki/Updating-beta-software). If you've set `$CHIA_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We have revamped the hddcoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `hddcoin-start-farmer &`. The [README.md](https://github.com/Chia-Network/hddcoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.hddcoin/release/config, plots/ db/, wallet/ etc. This can be overridden by `export CHIA_ROOT=~/.hddcoin` for example which would then put the plots directory in `HomeDirectory/.hddcoin/plots`.
- The libraries hddcoin-pos, hddcoin-fast-vdf, and hddcoin-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Chia-Network/chiapos), [chiavdf](https://github.com/Chia-Network/chiavdf), and [chaibip158](https://github.com/Chia-Network/chiabip158). They are brought in by hddcoin-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chia-Network/bls-signatures).
...
- Chia fullnode, farmer and harvester now default to logging to hddcoin.log in the hddcoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
[unreleased]: https://github.com/Chia-Network/hddcoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Chia-Network/hddcoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Chia-Network/hddcoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Chia-Network/hddcoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Chia-Network/hddcoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Chia-Network/hddcoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Chia-Network/hddcoin-blockchain/releases/tag/Alpha-1.0
