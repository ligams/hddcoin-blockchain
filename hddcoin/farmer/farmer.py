...
from hddcoin.plot_sync.delta import Delta
from hddcoin.plot_sync.receiver import Receiver
from hddcoin.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from hddcoin.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from hddcoin.util.errors import KeychainProxyConnectionFailure
...
from hddcoin.util.logging import TimedDuplicateFilter
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'hddcoin keys generate' or open the UI."
        try:
        ...
        if "hdd_target_address" not in self.config or "hdd_target_address" not in self.pool_config:
            log.debug("hdd_target_address missing in the config")
            return False
