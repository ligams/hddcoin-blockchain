...
from hddcoin.util.config import selected_network_address_prefix
...
def get_byte_per_unit(wallet_type: WalletType) -> int:  # pragma: no cover
    byte_per_unit: int
    if wallet_type in {
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["hddcoin"]))  # fees are always in HDD mojos
        ...
        if typ == WalletType.STANDARD_WALLET:
            ...
            res = await wallet_client.send_transaction(
                ...
                ).to_tx_config(byte_per_unit, config, fingerprint),
                final_fee,
                memos,
...
async def make_offer(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        fee: int = int(d_fee * units["hddcoin"])
            ...
            for item in [*offers, *requests]:
                ...
                offer, trade_record = await wallet_client.create_offer_for_ids(
                    ...
                    tx_config=CMDTXConfigLoader(
                        reuse_puzhash=reuse_puzhash,
                    ).to_tx_config(units["hddcoin"], config, fingerprint),
                )
                if offer is not None:
                    ...
                    print(
                        f"Use hddcoin wallet get_offers --id " f"{trade_record.trade_id} -f {fingerprint} to view status"
                    )
                else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_xch: str = "HDD"
) -> None:
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_xch = AddressType.HDD.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        ...
        if royalty_asset_dict != {}:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["hddcoin"] if summary["asset"] == network_xch else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["hddcoin"] if asset == network_xch else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['hddcoin']} {network_xch}, {offer.fees()} bytes")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["hddcoin"], config, fingerprint),
            )
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["hddcoin"], config, fingerprint), secure=secure, fee=fee
        )
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                ...
                coin_announcements,
                CMDTXConfigLoader().to_tx_config(units["hddcoin"], config, fingerprint),
            )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = (
            ...
            else ensure_valid_address(royalty_address, allowed_types={AddressType.HDD}, config=config)
        )
        target_address = (
            None
            if not target_address
            else ensure_valid_address(target_address, allowed_types={AddressType.HDD}, config=config)
        )
        fee: int = int(d_fee * units["hddcoin"])
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = ensure_valid_address(target_address, allowed_types={AddressType.HDD}, config=config)
            fee: int = int(d_fee * units["hddcoin"])
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["hddcoin"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["hddcoin"])
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.HDD:
            if address is None:
                print("Address is required for HDD address type.")
                return
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["hddcoin"]), force)
        print(str(response))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        vc_record, txs = await wallet_client.vc_mint(
            ...
            if target_address is None
                ...
                ensure_valid_address(target_address, allowed_types={AddressType.HDD}, config=config)
            ),
            uint64(int(d_fee * units["hddcoin"])),
        )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["hddcoin"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["hddcoin"], config, fingerprint),
        )
...
async def approve_r_cats(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        txs = await wallet_client.crcat_approve_pending(
            ...
            fee=uint64(int(fee * units["hddcoin"])),
            tx_config=CMDTXConfigLoader(
