...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, byte_per_unit: int) -> None:
    if verbose:
        print(tx)
    else:
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use hddcoin wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['hddcoin']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['hddcoin']}")
