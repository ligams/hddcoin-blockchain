...
from hddcoin.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from hddcoin.cmds.peer_funcs import print_connections
...
from hddcoin.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from hddcoin.util.config import load_config, selected_network_address_prefix
from hddcoin.util.default_root import DEFAULT_ROOT_PATH
from hddcoin.util.ints import uint16, uint32, uint64
from hddcoin.wallet.nft_wallet.nft_info import NFTInfo
...
from hddcoin.wallet.util.address_type import AddressType, ensure_valid_address
from hddcoin.wallet.util.puzzle_decorator_type import PuzzleDecoratorType
from hddcoin.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from hddcoin.wallet.util.transaction_type import CLAWBACK_TRANSACTION_TYPES, TransactionType
from hddcoin.wallet.util.wallet_types import WalletType
from hddcoin.wallet.vc_wallet.vc_store import VCProofs
...
def print_transaction(
    ...
    if verbose:
        ...
        hddcoin_amount = Decimal(int(tx.amount)) / byte_per_unit
        ...
        print(f"Amount {description}: {hddcoin_amount} {name}")
...
def get_byte_per_unit(wallet_type: WalletType) -> int:
    byte_per_unit: int
    if wallet_type in {
...
async def get_transaction(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    try:
        ...
        byte_per_unit = get_byte_per_unit(wallet_type=wallet_type)
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    try:
        ...
        byte_per_unit = get_byte_per_unit(typ)
    except LookupError:
        print(f"Wallet id: {wallet_id} not found.")
        return

    final_fee: uint64 = uint64(int(fee * units["hddcoin"]))  # fees are always in HDD bytes
    final_amount: uint64 = uint64(int(amount * byte_per_unit))
    final_min_coin_amount: uint64 = uint64(int(min_coin_amount * byte_per_unit))
    final_max_coin_amount: uint64 = uint64(int(max_coin_amount * byte_per_unit))
    ...
    print(f"To get status, use command: hddcoin wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["hddcoin"])
    ...
    if [] in [offers, requests]:
        ...
        for item in [*offers, *requests]:
                        ...
                        if item in requests:
                            ...
                            if info.supports_did:
                                ...
                                royalty_asset_dict[name] = (
                                    encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.HDD.hrp(config)),
            ...
            if fee > 0:
                print()
                print(f"Including Fees: {Decimal(fee) / units['hddcoin']} HDD, {fee} bytes")
            ...
            if royalty_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    ...
                    for summary in summaries:
                        divisor = units["hddcoin"] if summary["asset"] == "HDD" else units["cat"]
                        ...
                        print(
                            f"    - {converted_amount} {summary['asset']} ({summary['amount']} bytes) to {summary['address']}"  # noqa
                        )

                print()
                print("Total Amounts Offered:")
                for asset, requested_amount in total_amounts_requested.items():
                    divisor = units["hddcoin"] if asset == "HDD" else units["cat"]
                    converted_amount = Decimal(requested_amount) / divisor
                    print(f"  - {converted_amount} {asset} ({requested_amount} bytes)")
            ...
            if confirmation not in ["y", "yes"]:
                ...
                with open(pathlib.Path(filepath), "w") as file:
                    ...
                    if offer is not None:
                        ...
                        print(
                            f"Use hddcoin wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status"
                        )
                    else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_hdd="HDD"
):
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["hddcoin"]
        wid: str = "1" if asset_id == "hdd" else ""
        byte_amount: int = int(Decimal(amount))
        name: str = network_hdd
        if asset_id != "hdd":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
                ...
                if result is not None:
                    ...
                    name = result[1]
        output: str = f"    - {name}"
        byte_str: str = f"{byte_amount} {'byte' if byte_amount == 1 else 'bytes'}"
        if len(wid) > 0:
            output += f" (Wallet ID: {wid})"
        if unit == units["byte"]:
            output += f": {byte_str}"
        else:
            output += f": {byte_amount / unit} ({byte_str})"
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['hddcoin']} HDD, {fees} bytes")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    from hddcoin.util.bech32m import bech32_decode, convertbits
    from hddcoin.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    offered, requested, _ = offer.summary()
    cat_name_resolver = wallet_client.cat_asset_id_to_name
    network_hdd = AddressType.HDD.hrp(config).upper()
    print("Summary:")
    print("  OFFERED:")
    await print_offer_summary(cat_name_resolver, offered, network_hdd=network_hdd)
    print("  REQUESTED:")
    await print_offer_summary(cat_name_resolver, requested, network_hdd=network_hdd)
    ...
    for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
        if royalty_asset_id.hex() in offered:
            ...
            royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                encode_puzzle_hash(address, AddressType.HDD.hrp(config)),
                percentage,
            )
    ...
    if royalty_asset_dict != {}:
        ...
        for fungible_asset_id in fungible_assets_from_offer(offer):
            fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "hdd"
            if fungible_asset_id_str in requested:
                nft_royalty_currency: str = "Unknown CAT"
                if fungible_asset_id is None:
                    nft_royalty_currency = network_hdd
                else:
        ...
        if fungible_asset_dict != {}:
            ...
            for nft_id, summaries in royalty_summary.items():
                print(f"  - For {nft_id}:")
                for summary in summaries:
                    divisor = units["hddcoin"] if summary["asset"] == network_hdd else units["cat"]
                    converted_amount = Decimal(summary["amount"]) / divisor
                    total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                    total_amounts_requested[summary["asset"]] += summary["amount"]
                    print(
            ...
            for asset, amount in total_amounts_requested.items():
                divisor = units["hddcoin"] if asset == network_hdd else units["cat"]
                converted_amount = Decimal(amount) / divisor
                print(f"  - {converted_amount} {asset} ({amount} bytes)")

    print(f"Included Fees: {Decimal(offer.fees()) / units['hddcoin']} {network_hdd}, {offer.fees()} bytes")
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use hddcoin wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use hddcoin wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
async def print_balances(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if not is_syncing and is_synced:
        print(" ")
        print("HDDcoin HODL:")
        print(f"{indent}{'-Total Balance:'.ljust(23)} {hodl_balance_hdd} hdd ({hodl_balance_bytes} byte)")
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.HDD}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.HDD}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.HDD}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["hddcoin"])
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["hddcoin"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["hddcoin"])

    tx = await wallet_client.send_notification(address, message, amount, fee)

    print("Notification sent successfully.")
    print(f"To get status, use command: hddcoin wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def spend_clawback(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    if fee < 0:
        ...
        return
    response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["hddcoin"]))
    print(str(response))
...
async def mint_vc(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    vc_record, txs = await wallet_client.vc_mint(
        ...
        if args["target_address"] is None
            ...
            ensure_valid_address(args["target_address"], allowed_types={AddressType.HDD}, config=config)
        ),
        uint64(0) if args["fee"] is None else uint64(int(Decimal(args["fee"]) * units["hddcoin"])),
    )
    ...
    for tx in txs:
        print_transaction(
            ...
            name="HDD",
            address_prefix=selected_network_address_prefix(config),
            byte_per_unit=get_byte_per_unit(wallet_type=WalletType.STANDARD_WALLET),
        )
...
async def spend_vc(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:  # pragma: no cover
    ...
    txs = await wallet_client.vc_spend(
        ...
        fee=uint64(0) if args["fee"] is None else uint64(int(Decimal(args["fee"]) * units["hddcoin"])),
        reuse_puzhash=args["reuse_puzhash"],
    )
