...
from hddcoin.cmds import options
from hddcoin.cmds.check_wallet_db import help_text as check_help_text
from hddcoin.cmds.coins import coins_cmd
...
@wallet_cmd.command("send", help="Send hddcoin to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much HDD or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much HDD or CAT units",
    type=str,
    required=False,
    default=None,
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in HDD", default="0", show_default=True
)
@click.option(
    "--force",
...
@wallet_cmd.command("make_offer", help="Create an offer of HDD/CATs/NFTs for HDD/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in HDD", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in HDD", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck HDDcoin Wallet DBs"""

    from hddcoin.cmds.check_wallet_db import scan
...
@click.option(
    "-a",
    "--amount",
    help="Set the DID amount in bytes. Value must be an odd number.",
    type=int,
    default=1,
    show_default=True,
)
...
@click.option("-m", "--fee", help="The fee for the transaction, in HDD", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in HDD", type=str, required=False, default="0")
def mint_vc_cmd(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option(
    "-m", "--fee", help="Blockchain fee for update transaction, in HDD", type=str, required=False, default="0"
)
@click.option(
...
@click.option(
    "-m", "--fee", help="Blockchain fee for revocation transaction, in HDD", type=str, required=False, default="0"
)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
...
@click.option(
    "-m", "--fee", type=str, default=0, show_default=True, help="Blockchain fee for approval transaction, in HDD"
)
