...
from hddcoin.cmds.wallet_funcs import get_byte_per_unit, get_wallet_type, print_balance
...
async def async_list(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    try:
        ...
        byte_per_unit = get_byte_per_unit(wallet_type)
    except LookupError:
    ...
    final_excluded_amounts: List[uint64] = [uint64(int(Decimal(amount) * byte_per_unit)) for amount in excluded_amounts]
...
def print_coins(
    target_string: str, coins: List[Tuple[Coin, str]], byte_per_unit: int, addr_prefix: str, paginate: bool
) -> None:
    if len(coins) == 0:
        print("\tNo Coins.")
        return
    ...
    for i in range(0, len(coins), num_per_screen):
        for j in range(0, num_per_screen):
            ...
            amount_str = print_balance(coin.amount, byte_per_unit, "", decimal_only=True)
...
async def async_combine(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(Decimal(args["fee"]) * units["hddcoin"]))
    ...
    if not await wallet_client.get_synced():
        print("Wallet not synced. Please wait.")
        return
    is_hdd: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining HDDcoin
    final_max_amount = uint64(int(max_amount * byte_per_unit)) if not target_coin_ids else uint64(0)
    final_min_coin_amount: uint64 = uint64(int(min_coin_amount * byte_per_unit))
    final_excluded_amounts: List[uint64] = [uint64(int(Decimal(amount) * byte_per_unit)) for amount in excluded_amounts]
    final_target_coin_amount = uint64(int(target_coin_amount * byte_per_unit))
    if final_target_coin_amount != 0:  # if we have a set target, just use standard coin selection.
        removals: List[Coin] = await wallet_client.select_coins(
            amount=(final_target_coin_amount + final_fee) if is_hdd else final_target_coin_amount,
            wallet_id=wallet_id,
    ...
    if is_hdd and total_amount - final_fee <= 0:
    ...
    additions = [{"amount": (total_amount - final_fee) if is_hdd else total_amount, "puzzle_hash": target_ph}]
...
async def async_split(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if not await wallet_client.get_synced():
        print("Wallet not synced. Please wait.")
        return
    is_hdd: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting HDDcoin
    final_amount_per_coin = uint64(int(amount_per_coin * byte_per_unit))
    ...
    if removal_coin_record.coin.amount < total_amount:
        print(
            f"Coin amount: {removal_coin_record.coin.amount/ byte_per_unit} "
            f"is less than the total amount of the split: {total_amount/byte_per_unit}, exiting."
        )
