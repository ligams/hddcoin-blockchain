...
from hddcoin.cmds.cmds_util import CMDCoinSelectionConfigLoader, CMDTXConfigLoader, cli_confirm, get_wallet_client
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, _, config):
        ...
        conf_coins, unconfirmed_removals, unconfirmed_additions = await wallet_client.get_spendable_coins(
            ...
            ).to_coin_selection_config(byte_per_unit),
        )
...
async def async_split(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        print(f"Transaction sent: {tx_id}")
        print(f"To get status, use command: hddcoin wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
        dust_threshold = config.get("hdd_spam_amount", 1000000)  # min amount per coin in mojo
        ...
        if final_amount_per_coin < dust_threshold and wallet_type == WalletType.STANDARD_WALLET:
            print(
                ...
                f"{dust_threshold / byte_per_unit}. Some or all of the Coins "
