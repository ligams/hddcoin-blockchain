...
from hddcoin.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
...
from hddcoin.types.mempool_submission_status import MempoolSubmissionStatus
...
from hddcoin.util.keychain import KeyData
from hddcoin.util.streamable import Streamable, streamable
from hddcoin.wallet.transaction_record import TransactionRecord
from hddcoin.wallet.util.tx_config import CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig, TXConfigLoader
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'hddcoin wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'hddcoin keys generate' or import a key")
        ...
        click.option(
            ...
            click.option(
                ...
                click.option(
                    ...
                    "amounts_to_exclude",
                    multiple=True,
                    help="Exclude any coins with this HDD or CAT amount from being included.",
                )(func)
            )
        )
    )
...
class CMDCoinSelectionConfigLoader(Streamable):
    ...
    def to_coin_selection_config(self, byte_per_unit: int) -> CoinSelectionConfig:
        return CoinSelectionConfigLoader(
            uint64(int(Decimal(self.min_coin_amount) * byte_per_unit)) if self.min_coin_amount is not None else None,
            uint64(int(Decimal(self.max_coin_amount) * byte_per_unit)) if self.max_coin_amount is not None else None,
            [uint64(int(Decimal(a) * byte_per_unit)) for a in self.excluded_coin_amounts]
...
class CMDTXConfigLoader(CMDCoinSelectionConfigLoader):
    ...
    def to_tx_config(self, byte_per_unit: int, config: Dict[str, Any], fingerprint: int) -> TXConfig:
        cs_config = self.to_coin_selection_config(byte_per_unit)
