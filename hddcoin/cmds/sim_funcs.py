...
from hddcoin.cmds.start_funcs import async_start
from hddcoin.consensus.coinbase import create_puzzlehash_for_pk
from hddcoin.server.outbound_message import NodeType
from hddcoin.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from hddcoin.util.config import load_config, save_config, set_peer_info
...
def create_hddcoin_directory(
    ...
    """
    This function creates a new hddcoin directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from hddcoin.cmds.init_funcs import hddcoin_init

    if not hddcoin_root.is_dir() or not Path(hddcoin_root / "config" / "config.yaml").exists():
        # create hddcoin directories & load config
        hddcoin_init(hddcoin_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(hddcoin_root, "config.yaml")
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `hddcoin keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from hddcoin.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from hddcoin.simulator.block_tools import BlockTools, test_constants
    from hddcoin.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["HDDCOIN_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create hddcoin directory & get config.
    print("Creating hddcoin directory & config...")
    config = create_hddcoin_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "hddcoin")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your HDDCOIN_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from hddcoin.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically hddcoin show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['hddcoin']} THDD."
        )
        if show_addresses:
            print("All Addresses: ")
