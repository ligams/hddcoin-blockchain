...
from hddcoin.util.db_version import set_db_version
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        if "hdd_target_address" not in config["farmer"]:
            print(
                f"Setting the hdd destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def init(
    ...
    if create_certs is not None:
        ...
        return hddcoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def hddcoin_init(
    ...
    *,
    ...
    """
    hddcoin_root = os.environ.get("HDDCOIN_ROOT", None)
    if hddcoin_root is not None:
        print(f"HDDCOIN_ROOT is set to {hddcoin_root}")
