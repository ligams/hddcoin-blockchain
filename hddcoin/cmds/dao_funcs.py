...
from hddcoin.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from hddcoin.cmds.units import units
from hddcoin.cmds.wallet_funcs import get_byte_per_unit, get_wallet_type
...
from hddcoin.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.create_new_dao_wallet(
            ...
            tx_config=CMDTXConfigLoader.from_json_dict({"reuse_puzhash": True}).to_tx_config(
                units["hddcoin"], config, fingerprint
            ),
            dao_rules=None,
            amount_of_cats=None,
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["hddcoin"]))
    ...
    final_fee: uint64 = uint64(int(fee * units["hddcoin"]))

    fee_for_cat = Decimal(args["fee_for_cat"])
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["hddcoin"]))
...
async def get_treasury_balance(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        hdd_mojos = get_byte_per_unit(WalletType.STANDARD_WALLET)
        cat_mojos = get_byte_per_unit(WalletType.CAT)
        for asset_id, balance in balances.items():
            if asset_id == "xch":
                print(f"HDD: {balance / hdd_mojos}")
            else:
...
async def show_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, config):
        ...
        if ptype == "spend":
            hdd_conds = pd["hdd_conditions"]
            asset_conds = pd["asset_conditions"]
            print("")
            if hdd_conds:
                print("Proposal HDD Conditions")
                for pmt in hdd_conds:
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["hddcoin"]))
    proposal_id = args["proposal_id"]
...
async def create_spend_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_amount: Optional[uint64] = uint64(int(Decimal(amount) * byte_per_unit)) if amount else None
        ...
        if res["success"]:
            asset_id_name = asset_id if asset_id else "HDD"
