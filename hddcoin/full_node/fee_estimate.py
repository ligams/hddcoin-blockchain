...
from hddcoin.types.fee_rate import FeeRate, FeeRateV2
...
class FeeEstimate(Streamable):
    ...
    estimated_fee_rate: expressed in byte per 1 clvm_cost. `estimated_fee` can be zero.
    """

    error: Optional[str]
    time_target: uint64  # unix time stamp in seconds
    estimated_fee_rate: FeeRate  # Bytes per clvm cost
...
class FeeEstimateV2:
    error: Optional[str]
    time_target: uint64  # unix time stamp in seconds
    estimated_fee_rate: FeeRateV2  # Bytes per clvm cost


def fee_rate_v2_to_v1(fee_rate: FeeRateV2) -> FeeRate:
    return FeeRate(uint64(math.ceil(fee_rate.bytes_per_clvm_cost)))


def fee_estimate_v2_to_v1(fe: FeeEstimateV2) -> FeeEstimate:
    return FeeEstimate(fe.error, fe.time_target, FeeRate(uint64(math.ceil(fe.estimated_fee_rate.bytes_per_clvm_cost))))
...
class FeeEstimateGroup(Streamable):
    ...
    Estimates are expressed in bytes / 1 clvm_cost.
    """
