...
from hddcoin.consensus.blockchain import Blockchain, ReceiveBlockResult, StateChangeSummary
...
class FullNode:
    ...
    root_path: Path
    config: Dict[str, Any]
    _server: Optional[HDDcoinServer]
    _shut_down: bool
    ...
    def on_disconnect(self, connection: WSHDDcoinConnection) -> None:
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSHDDcoinConnection = self.server.all_connections[peer]
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSHDDcoinConnection
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSHDDcoinConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
