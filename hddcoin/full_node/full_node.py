...
from hddcoin.consensus.block_body_validation import ForkInfo
...
from hddcoin.consensus.blockchain import AddBlockResult, Blockchain, BlockchainMutexPriority, StateChangeSummary
...
from hddcoin.full_node.hint_management import get_hints_and_subscription_coin_ids
...
from hddcoin.full_node.subscriptions import PeerSubscriptions
from hddcoin.full_node.sync_store import Peak, SyncStore
from hddcoin.full_node.tx_processing_queue import TransactionQueue
...
from hddcoin.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
...
from hddcoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof, validate_vdf
...
from hddcoin.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from hddcoin.util.db_synchronous import db_synchronous_on
from hddcoin.util.db_version import lookup_db_version, set_db_version_async
from hddcoin.util.db_wrapper import DBWrapper2, manage_connection
from hddcoin.util.errors import ConsensusError, Err, TimestampError, ValidationError
from hddcoin.util.ints import uint8, uint32, uint64, uint128
from hddcoin.util.limited_semaphore import LimitedSemaphore
from hddcoin.util.log_exceptions import log_exceptions
from hddcoin.util.path import path_from_root
from hddcoin.util.profiler import mem_profile_task, profile_task
...
class FullNode:
    if TYPE_CHECKING:
        from hddcoin.rpc.rpc_server import RpcServiceProtocol
    ...
    _server: Optional[HDDcoinServer] = None
                    ...
                    async with conn.execute(
                        ...
                        if len(list(await cur.fetchall())) == 0:
                            try:
                                ...
                                # it could be a database created with "hddcoin init", which is
    ...
    async def short_sync_batch(self, peer: WSHDDcoinConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSHDDcoinConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSHDDcoinConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSHDDcoinConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSHDDcoinConnection) -> None:
        """
    ...
    async def on_disconnect(self, connection: WSHDDcoinConnection) -> None:
    ...
    async def request_validate_wp(
        ...
        weight_proof_peer: WSHDDcoinConnection = random.choice(peers_with_peak)
        self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSHDDcoinConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSHDDcoinConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSHDDcoinConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSHDDcoinConnection, List[FullBlock]]]]
        ) -> None:
            fork_info: Optional[ForkInfo] = None

            while True:
                res: Optional[Tuple[WSHDDcoinConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSHDDcoinConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSHDDcoinConnection]:
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSHDDcoinConnection] = None,
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSHDDcoinConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSHDDcoinConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSHDDcoinConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSHDDcoinConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSHDDcoinConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
