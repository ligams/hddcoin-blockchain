...
def call_plotters(root_path: Path, args):
    ...
    plotters = argparse.ArgumentParser("hddcoin plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, hddcoin_plotter_options, "chiapos", "Create a plot with the default hddcoin plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'hddcoin plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'hddcoin plotters bladebit2' was integrated to 'hddcoin plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        show_plotters_version(hddcoin_root_path)
