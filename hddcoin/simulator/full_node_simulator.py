...
from hddcoin.simulator.simulator_protocol import FarmNewBlockProtocol, GetAllCoinsProtocol, ReorgProtocol
...
from hddcoin.util.timing import adjusted_timeout, backoff_times
...
class FullNodeSimulator(FullNodeAPI):
    ...
    async def farm_blocks_to_puzzlehash(
        ...
        is to receive HDD at an address.
        ...
        Returns:
            The total number of reward bytes for the processed blocks.
        """
    ...
    async def farm_blocks_to_wallet(
        ...
        Returns:
            The total number of reward bytes farmed to the requested address.
        """
        if isinstance(timeout, _Default):
    ...
    async def farm_rewards_to_wallet(
        ...
        """Farm at least the requested amount of bytes to the passed wallet. Extra
        bytes will be received based on the block rewards at the present block height.
        The rewards will be present in the wall before returning.

        Arguments:
            amount: The minimum number of bytes to farm.
            wallet: The wallet to farm the block rewards to.

        Returns:
            The total number of reward bytes farmed to the requested wallet.
        """
        rewards = 0
    ...
    async def create_coins_with_amounts(
        ...
        Arguments:
            amounts: A list with entries of byte amounts corresponding to each
                coin to create.
