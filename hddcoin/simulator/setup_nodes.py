...
from hddcoin.daemon.server import WebSocketServer
...
from hddcoin.simulator.setup_services import (
    setup_daemon,
    setup_farmer,
    setup_full_node,
    setup_harvester,
...
SimulatorsAndWallets = Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, HDDcoinServer]], BlockTools]
...
) -> AsyncGenerator[Tuple[FullNodeAPI, FullNodeAPI, HDDcoinServer, HDDcoinServer, BlockTools], None]:
    """
...
) -> AsyncGenerator[Tuple[List[FullNodeAPI], List[Tuple[WalletNode, HDDcoinServer]], BlockTools], None]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system(
    ...
    db_version: int = 1,
) -> AsyncGenerator[Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord], object, object, Any, HDDcoinServer], None]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system_connect_to_deamon(
    ...
    Tuple[
        Any, Any, Harvester, Farmer, Any, Service[Timelord], object, object, Any, HDDcoinServer, Optional[WebSocketServer]
    ],
    None,
]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
