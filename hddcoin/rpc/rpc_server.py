...
from hddcoin.rpc.util import wrap_http_handler
from hddcoin.server.outbound_message import NodeType
from hddcoin.server.server import HDDcoinServer, ssl_context_for_client, ssl_context_for_server
...
from hddcoin.util.config import str2bool
from hddcoin.util.ints import uint16
from hddcoin.util.json_util import dict_to_json_str
from hddcoin.util.network import WebServer, resolve
from hddcoin.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> HDDcoinServer:
        ...
        # a property so as to be read only which allows HDDcoinServer to satisfy
        # Optional[HDDcoinServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        hddcoin.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: HDDcoinServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
