...
from hddcoin.types.signing_mode import SigningMode
...
from hddcoin.wallet.util.compute_hints import compute_coin_hints
...
from hddcoin.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords
...
class WalletRpcApi:
    ...
    async def spend_clawback_coins(self, request) -> EndpointResult:
        ...
        :param fee: transaction fee in bytes
        :return:
        """
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        if signing_mode == SigningMode.CHIP_0002:
            ...
            #   ("HDDcoin Signed Message", message)
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "hdd")
        ...
        key = None
        if asset_id is not None and asset_id != "hdd":
    ...
    async def nft_set_did_bulk(self, request):
        ...
        accepted `request` dict keys:
         ...
         - optional `fee`, in bytes, defaults to 0
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        hdd_coins = None
        if hdd_coin_list:
            hdd_coins = set([Coin.from_json_dict(hdd_coin) for hdd_coin in hdd_coin_list])
        hdd_change_target = request.get("hdd_change_target", None)
        if hdd_change_target is not None:
            if hdd_change_target[:2] == "hdd":
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_hdd(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
