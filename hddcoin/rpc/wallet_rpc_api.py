...
from hddcoin.data_layer.data_layer_errors import LauncherCoinNotFoundError
from hddcoin.data_layer.data_layer_wallet import DataLayerWallet
from hddcoin.pools.pool_wallet import PoolWallet
from hddcoin.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from hddcoin.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from hddcoin.rpc.util import tx_endpoint
...
from hddcoin.types.blockchain_format.coin import Coin, coin_as_list
...
from hddcoin.util.config import load_config, str2bool
from hddcoin.util.errors import KeychainIsLocked
from hddcoin.util.ints import uint16, uint32, uint64
from hddcoin.util.keychain import bytes_to_mnemonic, generate_mnemonic
from hddcoin.util.misc import UInt32Range
...
from hddcoin.wallet.cat_wallet.cat_info import CRCATInfo
from hddcoin.wallet.cat_wallet.cat_wallet import CATWallet
from hddcoin.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
...
from hddcoin.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from hddcoin.wallet.did_wallet.did_wallet import DIDWallet
from hddcoin.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
...
from hddcoin.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from hddcoin.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
...
from hddcoin.wallet.puzzles import p2_delegated_conditions
from hddcoin.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from hddcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
...
from hddcoin.wallet.util.address_type import AddressType, is_valid_address
...
from hddcoin.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from hddcoin.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from hddcoin.wallet.util.wallet_types import CoinType, WalletType
from hddcoin.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
...
from hddcoin.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSHDDcoinConnection, coin_id: bytes32, latest: bool = True
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `hddcoin keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from hddcoin.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.HDD.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        hdd_coin_list = request.get("hdd_coins", None)
        hdd_coins = None
        if hdd_coin_list:
            hdd_coins = {Coin.from_json_dict(hdd_coin) for hdd_coin in hdd_coin_list}
        hdd_change_target = request.get("hdd_change_target", None)
        if hdd_change_target is not None:
            if hdd_change_target[:2] == "xch":
                hdd_change_ph = decode_puzzle_hash(hdd_change_target)
            else:
                hdd_change_ph = bytes32(hexstr_to_bytes(hdd_change_target))
        else:
            hdd_change_ph = None
