...
if TYPE_CHECKING:
    ...
    class BaseEventsServer(asyncio.base_events.Server):
        if sys.platform == "win32" and sys.version_info >= (3, 8):
            _loop: HDDcoinProactorEventLoop
        else:
...
class PausableServer(BaseEventsServer):
    ...
    def _detach(self) -> None:
        ...
        if (
            ...
            self._hddcoin_resume()

    def _hddcoin_pause(self) -> None:
...
async def _hddcoin_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class HDDcoinSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _hddcoin_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class HDDcoinProactor(IocpProactor):
        ...
        async def _hddcoin_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._hddcoin_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _hddcoin_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: HDDcoinProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._hddcoin_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class HDDcoinProactorEventLoop(ProactorEventLoop):
...
class HDDcoinPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if sys.platform == "win32":
            if sys.version_info >= (3, 8):
                ...
                loop_factory = HDDcoinProactorEventLoop
            else:
                # separate condition so coverage can report when this is no longer used
                loop_factory = HDDcoinSelectorEventLoop
        else:
            loop_factory = HDDcoinSelectorEventLoop
...
def set_hddcoin_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(HDDcoinPolicy())
