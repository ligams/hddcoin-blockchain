...
from hddcoin.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from hddcoin.server.ws_connection import ConnectionCallback, WSHDDcoinConnection
...
from hddcoin.util.network import WebServer, is_in_network, is_localhost
...
class HDDcoinServer:
    ...
    all_connections: Dict[bytes32, WSHDDcoinConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        hddcoin_ca_crt_path, hddcoin_ca_key_path = hddcoin_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                hddcoin_ca_crt_path, hddcoin_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                hddcoin_ca_crt_path, hddcoin_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Optional[ConnectionCallback] = None) -> None:
        if self.webserver is not None:
            raise RuntimeError("HDDcoinServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSHDDcoinConnection.create(
                self._local_type,
                ws,
                self.api,
                self._port,
    ...
    async def connection_added(
        self, connection: WSHDDcoinConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSHDDcoinConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSHDDcoinConnection, trusted_peers: Dict[str, Any]) -> bool:
