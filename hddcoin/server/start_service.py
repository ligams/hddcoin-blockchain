...
from hddcoin.daemon.server import service_launch_lock_path
from hddcoin.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from hddcoin.server.hddcoin_policy import set_hddcoin_policy
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSHDDcoinConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"hddcoin-blockchain version: {hddcoin_full_version_str()}")
        ...
        self._server = HDDcoinServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"hddcoin_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_hddcoin_policy(connection_limit)
