...
from hddcoin.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from hddcoin.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from hddcoin.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from hddcoin.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
...
from hddcoin.wallet.util.merkle_utils import _simplify_merkle_proof
from hddcoin.wallet.util.transaction_type import TransactionType
from hddcoin.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from hddcoin.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSHDDcoinConnection) -> CoinState:
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, hddcoin_tx.spend_bundle])
                ),
                dataclasses.replace(hddcoin_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSHDDcoinConnection, coin_data: Optional[object]) -> None:
