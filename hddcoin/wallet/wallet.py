...
# https://github.com/HDDcoin-Network/chips/blob/80e4611fe52b174bf1a0382b9dff73805b18b8c6/CHIPs/chip-0002.md#signmessage
CHIP_0002_SIGN_MESSAGE_PREFIX = "HDDcoin Signed Message"
...
class Wallet:
    ...
    async def _generate_unsigned_transaction(
        ...
        if not ignore_max_send_amount:
            ...
            if total_amount > max_send:
                raise ValueError(f"Can't send more than {max_send} bytes in a single transaction")
            self.log.debug("Got back max send amount: %s", max_send)
        if coins is None:
            if total_amount > total_balance:
                raise ValueError(
                    f"Can't spend more than wallet balance: {total_balance} bytes, tried to spend: {total_amount} bytes"
                )
    ...
    async def create_spend_bundle_relative_hddcoin(self, hddcoin_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if hddcoin_amount < 0:
            utxos = await self.select_coins(uint64(abs(hddcoin_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(hddcoin_amount), "memos": []}
                ]
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSHDDcoinConnection
