...
from hddcoin.wallet.puzzles.clawback.metadata import ClawbackMetadata
...
from hddcoin.wallet.util.wallet_types import WalletIdentifier, WalletType
...
class Wallet:
    ...
    async def _generate_unsigned_transaction(
        ...
        if not ignore_max_send_amount:
            ...
            if total_amount > max_send:
                raise ValueError(f"Can't send more than {max_send} bytes in a single transaction, got {total_amount}")
            self.log.debug("Got back max send amount: %s", max_send)
        if coins is None:
            if total_amount > total_balance:
                raise ValueError(
                    f"Can't spend more than wallet balance: {total_balance} mojos, tried to spend: {total_amount} bytes"
                )
    ...
    async def create_tandem_hdd_tx(
        ...
        hddcoin_coins = await self.select_coins(fee, tx_config.coin_selection_config)
        [hddcoin_tx] = await self.generate_signed_transaction(
            uint64(0),
    ...
    # WSHDDcoinConnection is only imported for type checking
