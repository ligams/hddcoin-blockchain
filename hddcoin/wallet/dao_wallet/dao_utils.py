...
from hddcoin.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle
from hddcoin.wallet.dao_wallet.dao_info import DAORules, ProposalType
from hddcoin.wallet.puzzles.load_clvm import load_clvm
from hddcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import MOD
from hddcoin.wallet.singleton import get_singleton_struct_for_id
from hddcoin.wallet.uncurried_puzzle import UncurriedPuzzle
...
def get_spend_p2_singleton_puzzle(treasury_id: bytes32, hdd_conditions: Program, asset_conditions: Program) -> Program:
    # TODO: typecheck get_spend_p2_singleton_puzzle arguments
    # TODO: add tests for get_spend_p2_singleton_puzzle: pass hdd_conditions as Puzzle, List and ConditionWithArgs
    #

    # CAT_MOD_HASH
    # CONDITIONS  ; HDD conditions, to be generated by the treasury
...
def generate_simple_proposal_innerpuz(
    ...
    for recipient_puzhash, amount, asset_type in zip(recipient_puzhashes, amounts, asset_types):
        if asset_type:
            ...
            hdd_conds.append([51, recipient_puzhash, amount])
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(hdd_conds), Program.to(cat_conds))
    return puzzle
...
async def generate_mint_proposal_innerpuz(
    ...
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(hdd_conditions), Program.to([]))
    return puzzle
