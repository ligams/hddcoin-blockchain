...
from hddcoin.util.db_wrapper import manage_connection
from hddcoin.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from hddcoin.wallet.puzzles.clawback.metadata import AutoClaimSettings
from hddcoin.wallet.transaction_record import TransactionRecord
from hddcoin.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from hddcoin.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from hddcoin.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSHDDcoinConnection) -> PeerRequestCache:
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.hddcoin.org"]),
    ...
    async def on_disconnect(self, peer: WSHDDcoinConnection) -> None:
    ...
    def is_trusted(self, peer: WSHDDcoinConnection) -> bool:
        return self.server.is_trusted_peer(peer, self.config.get("trusted_peers", {}))

    async def state_update_received(self, request: CoinStateUpdate, peer: WSHDDcoinConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSHDDcoinConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSHDDcoinConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSHDDcoinConnection]:
        """
        ...
        synced_and_trusted: List[WSHDDcoinConnection] = []
        synced: List[WSHDDcoinConnection] = []
        trusted: List[WSHDDcoinConnection] = []
        neither: List[WSHDDcoinConnection] = []
        all_nodes: List[WSHDDcoinConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSHDDcoinConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSHDDcoinConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSHDDcoinConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSHDDcoinConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSHDDcoinConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSHDDcoinConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSHDDcoinConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSHDDcoinConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSHDDcoinConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSHDDcoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSHDDcoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
