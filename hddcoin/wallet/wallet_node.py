...
from hddcoin.protocols.wallet_protocol import CoinState, RespondBlockHeader, RespondToCoinUpdates
...
from hddcoin.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
...
class WalletNode:
    ...
    async def is_peer_synced(
        self, peer: WSHDDcoinConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSHDDcoinConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSHDDcoinConnection]:
        """
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSHDDcoinConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSHDDcoinConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSHDDcoinConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSHDDcoinConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSHDDcoinConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSHDDcoinConnection) -> CoinSpend:
