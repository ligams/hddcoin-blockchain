...
from hddcoin.wallet.did_wallet.did_wallet_puzzles import create_fullpuz, uncurry_innerpuz
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSHDDcoinConnection):
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            hddcoin_tx = await self.create_tandem_hdd_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
