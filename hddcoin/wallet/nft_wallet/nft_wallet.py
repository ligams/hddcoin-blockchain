...
import hddcoin.wallet.singleton
...
from hddcoin.wallet.nft_wallet import nft_puzzles
from hddcoin.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from hddcoin.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from hddcoin.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from hddcoin.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from hddcoin.wallet.payment import Payment
from hddcoin.wallet.puzzle_drivers import PuzzleInfo, Solver
from hddcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from hddcoin.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from hddcoin.wallet.util.wallet_sync_utils import fetch_coin_spend
from hddcoin.wallet.util.wallet_types import WalletType
from hddcoin.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
...
class NFTWallet:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSHDDcoinConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        if hddcoin_tx is not None and hddcoin_tx.spend_bundle is not None:
            ...
            hddcoin_tx = dataclasses.replace(hddcoin_tx, spend_bundle=None)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            hddcoin_tx = await self.standard_wallet.create_tandem_hdd_tx(
        ...
        return nft_spend_bundle, hddcoin_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'HDD'} to offer")
    ...
    async def mint_from_did(
        ...
        hdd_coins: Optional[Set[Coin]] = None,
        hdd_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an hdd spend that funds the transactions and pays fees.
        ...
        :param hdd_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param hdd_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the hdd spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an hdd coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if hdd_coins is None:
            hdd_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(hdd_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                hddcoin.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), hddcoin.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, hddcoin.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the hdd spend to fund the minting.
        spend_value = sum([coin.amount for coin in hdd_coins])
        change: uint64 = uint64(spend_value - total_amount)
        hdd_spends = []
        if hdd_change_ph is None:
            hdd_change_ph = await self.standard_wallet.get_new_puzzlehash()
        hdd_payment = Payment(hdd_change_ph, change, [hdd_change_ph])

        first = True
        for hdd_coin in hdd_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(hdd_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in hdd_coins]
                message_list.append(Coin(hdd_coin.name(), hdd_payment.puzzle_hash, hdd_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(hdd_coins) > 1:
                    hdd_announcement: Optional[Set[bytes]] = {message}
                else:
                    hdd_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[hdd_payment],
                    fee=fee,
                    coin_announcements=hdd_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(hdd_coin.name(), message).name()
            ...
            hdd_spends.append(CoinSpend(hdd_coin, puzzle, solution))
        hdd_spend = await self.standard_wallet.sign_transaction(hdd_spends)
        ...
        did_full_puzzle: Program = hddcoin.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, hdd_spend, *eve_spends])
        return total_spend

    async def mint_from_hdd(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single HDD spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = hdd_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from HDD to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the HDD spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the HDD spend
            ...
            # Annnouncements for eve spend. These are asserted by the hdd spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for hdd_coin in hdd_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[hdd_payment] + primaries,
                    fee=fee,
                    coin_announcements=hdd_announcement if len(hdd_coins) > 1 else None,
