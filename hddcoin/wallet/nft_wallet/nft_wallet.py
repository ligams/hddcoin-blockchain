...
from hddcoin.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
...
from hddcoin.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
...
from hddcoin.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSHDDcoinConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSHDDcoinConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if hdd_coins is None:
            hdd_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        ...
        for hdd_coin in hdd_coins:
            ...
            hdd_spends.append(make_spend(hdd_coin, puzzle, solution))
        hdd_spend = await self.wallet_state_manager.sign_transaction(hdd_spends)
