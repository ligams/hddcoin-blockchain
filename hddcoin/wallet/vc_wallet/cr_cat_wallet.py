...
from hddcoin.util.misc import VersionedBlob
from hddcoin.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from hddcoin.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from hddcoin.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSHDDcoinConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_hddcoin_transaction:
                    if fee > regular_hddcoin_to_claim:
                        ...
                        hddcoin_tx, hdd_announcement = await self.create_tandem_hdd_tx(
                            fee,
                            uint64(regular_hddcoin_to_claim),
                            tx_config,
                        )
                        assert hdd_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={hdd_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [hddcoin_tx.spend_bundle]
            ...
            [*vc_txs, *([hddcoin_tx] if hddcoin_tx is not None else [])],
        )
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if not ignore_max_send_amount:
            ...
            if payment_sum > max_send:
                raise ValueError(f"Can't send more than {max_send} bytes in a single transaction")  # pragma: no cover
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(hddcoin_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if hddcoin_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, hddcoin_tx.spend_bundle])
        else:
            hddcoin_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(hddcoin_tx, spend_bundle=None),) if hddcoin_tx is not None else []),
        ]
