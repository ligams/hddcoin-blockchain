...
from hddcoin.wallet.vc_wallet.vc_store import VCRecord, VCStore
...
class VCWallet:
    ...
    async def revoke_vc(
        self, parent_id: bytes32, peer: WSHDDcoinConnection, fee: uint64 = uint64(0), reuse_puzhash: Optional[bool] = None
    ) -> List[TransactionRecord]:
        ...
        if fee > 0:
            ...
            tx = dataclasses.replace(tx, spend_bundle=SpendBundle.aggregate([hddcoin_tx.spend_bundle, tx.spend_bundle]))
            hddcoin_tx = dataclasses.replace(hddcoin_tx, spend_bundle=None)
            return [tx, hddcoin_tx]
        else:
...
if TYPE_CHECKING:
    from hddcoin.wallet.wallet_protocol import WalletProtocol  # pragma: no cover
