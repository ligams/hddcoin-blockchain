...
from hddcoin.cmds.init_funcs import check_keys, hddcoin_full_version_str, hddcoin_init
...
from hddcoin.util.beta_metrics import BetaMetricsLogger
...
from hddcoin.util.keychain import Keychain, passphrase_requirements, supports_os_passphrase_storage
from hddcoin.util.lock import Lockfile, LockfileError
from hddcoin.util.network import WebServer
...
if getattr(sys, "frozen", False):
    name_map = {
        "hddcoin": "hddcoin",
        "hddcoin_data_layer": "start_data_layer",
        "hddcoin_data_layer_http": "start_data_layer_http",
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of HDDcoin will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        if plotter == "bladebit":
            ...
            # 'hddcoin plotters bladebit ramplot' or 'hddcoin plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def exit(self) -> None:
        ...
        log.info("hddcoin daemon exiting")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with HDDCOIN_ROOT: {os.environ['HDDCOIN_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"hddcoin-blockchain version: {hddcoin_full_version_str()}")
