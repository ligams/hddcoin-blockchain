...
export default function ClawbackClaimTransactionDialog(props: Props) {
  ...
  const { onClose, open, coinId, amountInByte, fromOrTo, address } = props;
  ...
  const autoClaimFee = autoClaimData?.txFee ? byteToHDDcoin(autoClaimData.txFee) : 0;
  ...
  async function handleSubmit(values: FormData) {
    ...
    const response = await spendClawbackCoins({ coinIds: [coinId], fee: feeInBytes }).unwrap();
    if (response.transactionIds.length === 0) {
      throw new Error('No transaction ids returned');
    }

    if (shouldEnableAutoClaim && feeInBytes > 0) {
            ...
            <Flex gap={2} flexDirection="column" sx={{ textAlign: 'center', alignItems: 'center' }}>
              <Box sx={{ mb: 3 }}>
                <Typography variant="h5">
                  <FormatLargeNumber value={byteToHDDcoin(amountInByte)} />{' '}
