import { TransactionType, WalletType } from '@hddcoin-network/api';
import type { Transaction } from '@hddcoin-network/api';
import { useGetWalletBalanceQuery } from '@hddcoin-network/api-react';
import { byteToHDDcoin, byteToCAT, blockHeightToTimestamp } from '@hddcoin-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? byteToCAT(start) : byteToHDDcoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? byteToCAT(balance) : byteToHDDcoin(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? byteToCAT(start) : byteToHDDcoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? byteToCAT(start) : byteToHDDcoin(start).toString(), // bignumber is not supported by react
    });
  });
