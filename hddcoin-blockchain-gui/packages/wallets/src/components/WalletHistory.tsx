import { WalletType, TransactionType, toBech32m } from '@hddcoin-network/api';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@hddcoin-network/api-react';
import {
  Card,
  CopyToClipboard,
  Flex,
...
import type { Row } from '@hddcoin-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? byteToCAT(row.amount) : byteToHDDcoin(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={byteToHDDcoin(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
