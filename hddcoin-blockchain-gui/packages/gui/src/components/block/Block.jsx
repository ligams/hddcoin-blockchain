import { toBech32m } from '@hddcoin-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@hddcoin-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const baseFarmerReward = byteToHDDcoin(calculateBaseFarmerReward(blockRecord.height));

  const hddcoinFees = blockRecord.fees !== undefined ? byteToHDDcoin(blockRecord.fees) : '';
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the HDDcoin blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
