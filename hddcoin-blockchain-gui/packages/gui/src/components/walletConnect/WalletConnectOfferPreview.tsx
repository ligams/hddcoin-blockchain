import { Button, Flex, useOpenDialog, hddcoinToByte } from '@hddcoin-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeHDDcoin = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeHDDcoin) {
        const feeBytes = feeHDDcoin ? hddcoinToByte(feeHDDcoin).toFixed() : '0';
        onChange({
          ...values,
          fee: feeBytes,
        });
