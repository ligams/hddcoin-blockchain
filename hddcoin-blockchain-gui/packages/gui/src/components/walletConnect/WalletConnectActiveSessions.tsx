import { Flex, Loading, TooltipIcon } from '@hddcoin-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `hddcoin_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.hddcoin?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the HDDcoin application may not support some commands requested by the
