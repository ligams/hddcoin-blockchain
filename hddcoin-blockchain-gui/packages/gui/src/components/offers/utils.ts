...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@hddcoin-network/api';
import { byteToCAT, byteToHDDcoin, byteToCATLocaleString, byteToHDDcoinLocaleString } from '@hddcoin-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> HDD assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return byteToHDDcoinLocaleString(amount, locale);
  }
  if (walletType === WalletType.CAT) {
    return byteToCATLocaleString(amount, locale);
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.HDDCOIN]) {
    ...
    if (assetId) {
      const amountInBytes = offerAssetAmountForAssetId(assetId, summary);
      if (amountInBytes) {
        const amountInTokens = assetType === OfferAsset.HDDCOIN ? byteToHDDcoin(amountInBytes) : byteToCAT(amountInBytes);
