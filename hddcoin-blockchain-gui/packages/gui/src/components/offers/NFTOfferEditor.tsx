...
/*       An NFT offer consists of a single NFT being offered for HDD/CAT      */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                recommended minimum fee is 0.000005 HDD (5,000,000 bytes)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
    ...
    if (
      ...
      switch (tokenWalletInfo.walletType) {
        case WalletType.STANDARD_WALLET:
          balanceString = byteToHDDcoinLocaleString(walletBalance.spendableBalance, locale);
          balance = byteToHDDcoin(walletBalance.spendableBalance);
          break;
        case WalletType.CAT:
          balanceString = byteToCATLocaleString(walletBalance.spendableBalance, locale);
          balance = byteToCAT(walletBalance.spendableBalance);
          break;
        default:
          break;
      }
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*           Currently only supports a single NFT <--> HDD/CAT offer          */
/* ========================================================================== */
...
function buildOfferRequest(params: NFTBuildOfferRequestParams) {
  ...
  const baseByteAmount: BigNumber =
    tokenWalletInfo.walletType === WalletType.CAT ? catToByte(tokenAmount) : hddcoinToByte(tokenAmount);
  const byteAmount = exchangeType === NFTOfferExchangeType.NFTForToken ? baseByteAmount : baseByteAmount.negated();
  const feeByteAmount = hddcoinToByte(fee);
  ...
  return [
    {
      ...
      [tokenWalletInfo.walletId]: byteAmount,
    },
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const [offer, driverDict, feeInBytes] = buildOfferRequest({
      exchangeType: exchangeTypeLocal,
      nft: offerNFT,
