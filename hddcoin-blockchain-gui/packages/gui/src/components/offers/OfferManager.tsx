...
import { useCancelOfferMutation, useGetWalletsQuery } from '@hddcoin-network/api-react';
import {
  Button,
  ButtonLoading,
  Card,
...
export function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: hddFee } = methods.getValues();

    const fee = cancelWithTransaction ? hddcoinToByte(hddFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'THDD';
                  ...
                  <Flex flexDirection="column" gap={4} height="100%">
                    <Flex flexDirection="column" gap={2} flexGrow={1}>
                      ...
                      <Typography variant="body1" color="textSecondary">
                        <Trans>
                          Create a file that you can use to trade HDD, HDDcoin Asset Tokens, or NFTs with no counter-party
                          risk.
                        </Trans>
                      </Typography>
                    </Flex>
