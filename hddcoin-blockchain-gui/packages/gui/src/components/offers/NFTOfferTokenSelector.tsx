...
import type { CATToken, Wallet } from '@hddcoin-network/api';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const hddcoinWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'HDDcoin',
        symbol: currencyCode,
        displayName: `HDDcoin (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...hddcoinWalletSelection, ...catOptions];
