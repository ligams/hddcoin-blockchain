import { type OfferSummaryRecord } from '@hddcoin-network/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, byteToHDDcoin, byteToCAT } from '@hddcoin-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['hdd', 'thdd'].includes(makerEntries[0][0].toLowerCase())
        ? byteToHDDcoin(makerEntries[0][1])
        : byteToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['hdd', 'thdd'].includes(takerEntries[0][0].toLowerCase())
        ? byteToHDDcoin(takerEntries[0][1])
        : byteToCAT(takerEntries[0][1])
      : undefined;
