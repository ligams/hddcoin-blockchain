...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'HDD').toUpperCase();
  const isTestnet = currencyCode === 'THDD';
  ...
  const spendableBalance = byteToHDDcoinLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.hddcoin.org' : 'faucet.hddcoin.org'}/?address=${currentAddress}`;
    openExternal(url);
  }

  async function handleSubmit(data: CreateProfileData) {
    if (!canCreateProfile) {
      throw new Error(t`Your spendable balance must be greater than 1 byte to create a profile`);
    }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Bytes from the HDDcoin Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
                <Typography variant="body1" sx={{ fontWeight: 500 }}>
                  <Trans>Creating a profile requires 1 byte</Trans>
                </Typography>
