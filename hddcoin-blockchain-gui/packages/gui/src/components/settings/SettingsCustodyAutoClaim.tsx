import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@hddcoin-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, hddcoinToByte, byteToHDDcoin } from '@hddcoin-network/core';
import { ConnectCheckmark } from '@hddcoin-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? byteToHDDcoin(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInBytes = hddcoinToByte(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInBytes,
      minAmount: feeInBytes,
      batchSize: 50,
    }).unwrap();
