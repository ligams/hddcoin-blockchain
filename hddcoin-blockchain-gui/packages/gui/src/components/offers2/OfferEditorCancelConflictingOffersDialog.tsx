import { ConfirmDialog, Flex } from '@hddcoin-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'HDD' && assetStatus.type === 'HDD') {
      // assetId might be either 'HDD' or 'UNKNOWN'(maybe fee) but here both are treated as just a HDD spending
