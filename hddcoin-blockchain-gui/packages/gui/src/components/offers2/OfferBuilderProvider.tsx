import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@hddcoin-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@hddcoin-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@hddcoin-network/api-react';
import { catToByte, hddcoinToByte } from '@hddcoin-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedHDD = useWatch({
    name: 'requested.hdd',
  });

  const offeredHDD = useWatch({
    name: 'offered.hdd',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedHDD ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('hdd', hddcoinToByte(amount))),
    ...(requestedTokens ?? [])
      .filter(({ assetId, amount }) => assetId?.length > 0 && amount > 0)
      .map(({ amount, assetId }) => fungibleAssetFromAssetIdAndAmount(assetId, catToByte(amount))),
  ];
