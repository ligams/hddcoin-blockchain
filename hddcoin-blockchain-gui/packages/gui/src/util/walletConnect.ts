...
const log = initDebug('hddcoin-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `hddcoin_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.hddcoin;
    if (!requiredNamespace) {
      throw new Error('Missing required hddcoin namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['hddcoin:testnet', 'hddcoin:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `hddcoin:${instance}:${fingerprint}`);
