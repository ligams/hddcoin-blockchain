...
export function getUserDataDir(): string {
  const hddcoinRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(hddcoinRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const hddcoinRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${hddcoinRootUserDataPath}`);
  app.setPath('userData', hddcoinRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(hddcoinRootUserDataPath, 'Local Storage', 'leveldb');
