...
test('Confirm that User cannot send a THDD amount greater then in Wallet', async () => {
  let receive_wallet = 'thdd1ksr59en3j3t9zaprmya0jp9k6mkazq2u5lfccaf85sej7kgmvjjscanfxd';
  ...
  console.log(`HDD Balance: ${balance}`);
  ...
  //await page.locator('text=Fee *THDD >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.locator('div[role="dialog"]')).toHaveText(
    ...
    `ErrorCan't send more than ${haveBalanceString} bytes in a single transaction, got 200000000000000OK`
  );
  ...
  await expect(page.getByText('Pending Balance0 THDD')).toBeVisible();
});
