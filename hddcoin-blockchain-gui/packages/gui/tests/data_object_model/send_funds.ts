...
export class SendFunds {
  ...
  async send(receive_wallet: string, amount: string) {
    ...
    //await this.page.locator('text=Fee *THDD >> input[type="text"]').fill(fee);
    //await this.page.getByRole('button', { name: '0 (>5 min) THDD' }).click();
    //await this.page.getByRole('option', { name: '0.00000275276505264396 THDD Likely in 60 seconds' }).getByText('0.00000275276505264396 THDD').click();
  ...
  async check_balance() {
    if (this.page.locator('text=Spendable Balance0 THDD >> h5')) {
