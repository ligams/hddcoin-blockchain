...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.hddcoin.org/latest/latest.json'
  );
    ...
    ipcRenderer.invoke('fetchHtmlContent', latestVersionURL).then((obj: any) => {
      try {
        ...
        /* we don't need to handle error here, if we are unable to fetch version number
           from hddcoin.org, we just ignore showing reminder dialog */
      }
    });
  }, [latestVersionURL]);

  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.hddcoin.org/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.hddcoin.org/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.hddcoin.org/').toString() : undefined;
