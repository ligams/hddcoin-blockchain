...
export default function SettingsApp(props: SettingsAppProps) {
  ...
  async function handleOpenFAQURL(): Promise<void> {
    try {
      ...
      await shell.openExternal('https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/FAQ');
    } catch (error: any) {
      showError(error);
    }
  }
  ...
  async function handleOpenSendFeedbackURL(): Promise<void> {
    try {
      const { shell } = window as unknown as { shell: Shell };
      await shell.openExternal('https://github.com/HDDcoin-Network/hddcoin-blockchain/issues');
    } catch (error: any) {
      showError(error);
    }
  }
	 ...
	 {/* 
      <Flex flexDirection="column" gap={1}>
        <Flex flexDirection="row" gap={1}>
          <SettingsLabel>
            <Trans>HDDcoin Application Version:</Trans>
          </SettingsLabel>
          {version && (
