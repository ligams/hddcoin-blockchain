import { useGetFeeEstimateQuery } from '@hddcoin-network/api-react';
...
import byteToHDDcoinLocaleString from '../../utils/byteToHDDcoinLocaleString';
import Fee from '../Fee';
  ...
  useEffect(() => {
    if (formattedEstimates) {
      if (selectedTime) {
        ...
        if (estimate) {
          const hddFee = byteToHDDcoinLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, hddFee);
        }
      }
    }
