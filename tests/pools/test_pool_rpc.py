...
from hddcoin.simulator.setup_nodes import setup_simulators_and_wallets_service
...
from hddcoin.wallet.derive_keys import find_authentication_sk, find_owner_sk
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_self(
        ...
        async with manage_temporary_pool_plot(bt, status.p2_singleton_puzzle_hash) as pool_plot:
            ...
            tr: TransactionRecord = await client.send_transaction(
                1, uint64(100), encode_puzzle_hash(status.p2_singleton_puzzle_hash, "thdd")
            )
    ...
    async def test_leave_pool(
        ...
        try:
            ...
            async def have_hddcoin() -> bool:
                return (await wallets[0].get_confirmed_balance()) > FEE_AMOUNT

            await time_out_assert(timeout=MAX_WAIT_SECS, function=have_hddcoin)
