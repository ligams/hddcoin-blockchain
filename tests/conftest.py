...
from hddcoin.seeder.dns_server import DNSServer
...
from hddcoin.simulator.setup_services import setup_crawler, setup_daemon, setup_introducer, setup_seeder, setup_timelord
...
from hddcoin.types.aliases import (
    CrawlerService,
    DataLayerService,
    FarmerService,
    FullNodeService,
...
from hddcoin.util.db_wrapper import generate_in_memory_db_uri
...
from hddcoin.simulator.block_tools import BlockTools, create_block_tools_async, test_constants
...
def pytest_collection_modifyitems(session, config: pytest.Config, items: List[pytest.Function]):
    ...
    if len(all_error_lines) > 0:
        all_error_lines.insert(0, "custom hddcoin collection rules failed")
...
) -> AsyncIterator[Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, HDDcoinServer]], BlockTools]]:
...
async def two_wallet_nodes_custom_spam_filtering(
    spam_filter_after_n_txs, hdd_spam_amount, blockchain_constants: ConsensusConstants
):
    async with setup_simulators_and_wallets(
        1, 2, blockchain_constants, spam_filter_after_n_txs, hdd_spam_amount
    ) as new:
        yield make_old_setup_simulators_and_wallets(new=new)
...
def hddcoin_root_fixture(tmp_path: Path, scripts_path: Path) -> ChiaRoot:
    root = ChiaRoot(path=tmp_path.joinpath("hddcoin_root"), scripts_path=scripts_path)
    root.run(args=["init"])
