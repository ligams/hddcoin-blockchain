...
from hddcoin.simulator.setup_nodes import (
...
from hddcoin.simulator.setup_services import setup_daemon, setup_introducer, setup_timelord
...
from hddcoin.util.config import create_default_hddcoin_config, lock_and_load_config
from hddcoin.util.ints import uint16, uint64
from hddcoin.util.task_timing import main as task_instrumentation_main
from hddcoin.util.task_timing import start_task_instrumentation, stop_task_instrumentation
...
from hddcoin.simulator.block_tools import BlockTools, create_block_tools, create_block_tools_async, test_constants
from hddcoin.simulator.keyring import TempKeyring
from hddcoin.simulator.setup_nodes import setup_farmer_multi_harvester
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the hddcoin root directory
...
async def empty_blockchain(request):
    ...
    """
    from hddcoin.simulator.setup_nodes import test_constants
...
async def wallet_node_sim_and_wallet() -> AsyncIterator[
    Tuple[List[Union[FullNodeAPI, FullNodeSimulator]], List[Tuple[Wallet, HDDcoinServer]], BlockTools],
]:
...
async def simulator_and_wallet() -> AsyncIterator[
    Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, HDDcoinServer]], BlockTools]
]:
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, hdd_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, hdd_spam_amount):
        yield _
...
@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], HDDcoinServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def empty_keyring():
    with TempKeyring(user="user-hddcoin-1.8", service="hddcoin-user-hddcoin-1.8") as keychain:
        yield keychain
        KeyringWrapper.cleanup_shared_instance()
...
@pytest.fixture(scope="function")
def tmp_hddcoin_root(tmp_path):
    """
    Create a temp directory and populate it with an empty hddcoin_root directory.
    """
    path: Path = tmp_path / "hddcoin_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_hddcoin_root) -> Path:
    """
    Create a temp hddcoin_root directory and populate it with a default config.yaml.
    Returns the hddcoin_root path.
    """
    root_path: Path = tmp_hddcoin_root
...
@pytest.fixture(name="hddcoin_root", scope="function")
def hddcoin_root_fixture(tmp_path: Path, scripts_path: Path) -> HDDcoinRoot:
    root = HDDcoinRoot(path=tmp_path.joinpath("hddcoin_root"), scripts_path=scripts_path)
    root.run(args=["init"])
