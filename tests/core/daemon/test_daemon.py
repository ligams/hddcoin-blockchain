...
from hddcoin.daemon.server import WebSocketServer, plotter_log_path, service_plotter
...
from hddcoin.simulator.setup_services import setup_full_node
from hddcoin.util.config import load_config
from hddcoin.util.json_util import dict_to_json_str
from hddcoin.util.keychain import Keychain, KeyData, supports_os_passphrase_storage
from hddcoin.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE, KeyringWrapper
from hddcoin.util.ws_message import create_payload, create_payload_dict
from hddcoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk
...
def mock_daemon_with_services():
    ...
    return Daemon(
        services={
            ...
            "hddcoin_plotter": [Service(True), Service(True)],
        },
        connections={},
        net_config={},
    )
    ...
    async with aiohttp.ClientSession() as client:
        async with client.ws_connect(
            ...
            async with setup_full_node(
                ...
                await time_out_assert_not_none(30, ws_server.connections.get, "hddcoin_full_node")

                payload = create_payload("get_blockchain_state", {}, service_name, "hddcoin_full_node")
                await ws.send_str(payload)

                response = await ws.receive()
                message = assert_response_success_only(response)
                assert message["command"] == "get_blockchain_state"
                assert message["origin"] == "hddcoin_full_node"
...
@datacases(
    ...
    RouteCase(
        route="register_service",
        description="hddcoin_plotter",
        request={
            "service": "hddcoin_plotter",
        },
        response={"success": True, "service": "hddcoin_plotter", "queue": []},
    ),
    RouteCase(
...
async def test_plotter_options(
    ...
    # register for hddcoin_plotter events
    service_name = "hddcoin_plotter"
    data = {"service": service_name}
    payload = create_payload("register_service", data, "hddcoin_plotter", "daemon")
    await ws.send_str(payload)
    response = await ws.receive()
...
def test_run_plotter_bladebit(
    ...
    mocker.patch("hddcoin.plotters.bladebit.get_bladebit_version", side_effect=get_bladebit_version)
    mock_run_plotter = mocker.patch("hddcoin.plotters.bladebit.run_plotter")
