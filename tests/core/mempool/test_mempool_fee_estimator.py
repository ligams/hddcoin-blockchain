...
from hddcoin.full_node.bitcoin_fee_estimator import BitcoinFeeEstimator
from hddcoin.full_node.coin_store import CoinStore
from hddcoin.full_node.fee_estimate_store import FeeStore
from hddcoin.full_node.fee_estimation import MempoolItemInfo
from hddcoin.full_node.fee_estimator import SmartFeeEstimator
from hddcoin.full_node.fee_tracker import FeeTracker
...
async def test_fee_increase() -> None:
    async with DBConnection(db_version=2) as db_wrapper:
        ...
        assert short_estimate.bytes_per_clvm_cost == uint64(fee_tracker.buckets[3] / 1000)
        assert med_estimate.bytes_per_clvm_cost == uint64(fee_tracker.buckets[3] / 1000)
        assert long_estimate.bytes_per_clvm_cost == uint64(0)
