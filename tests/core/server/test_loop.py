...
from hddcoin.server import hddcoin_policy
...
class ServeInThread:
    ...
    def start(self) -> None:
        self.original_connection_limit = hddcoin_policy.global_max_concurrent_connections
        # TODO: yuck yuck, messes with a single global
        hddcoin_policy.global_max_concurrent_connections = self.connection_limit
    ...
    def _run(self) -> None:
        ...
        asyncio.set_event_loop_policy(hddcoin_policy.HDDcoinPolicy())
        try:
    ...
    def stop(self) -> None:
        ...
        if self.original_connection_limit is not None:
            hddcoin_policy.global_max_concurrent_connections = self.original_connection_limit
...
async def test_loop() -> None:
    ...
    for line in output.splitlines():
        ...
        # mark = "HDDcoinProactor._hddcoin_accept_loop() entering count="
        # if mark in line:
        #     _, _, rest = line.partition(mark)
        #     count = int(rest)
        #     if count > 1:
