...
std_farming_address = "thdd1mh4qanzyawn3v4uphgaj2cg6hrjazwyp0sx653fhn9apg6mfajlqtj0ztp"
burn_address = "thdd1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqm6ksh7qddh"  # 0x0...dead
...
def test_every_simulator_command() -> None:
    ...
    assert "hddcoin_full_node_simulator: started" in start_result.output
...
def stop_simulator(runner: CliRunner, simulator_name: str) -> None:
    ...
    assert "hddcoin_full_node_simulator: Stopped\nDaemon stopped\n" == result.output
...
def _test_sim_status(runner: CliRunner, address: str, simulator_name: str) -> None:
    ...
    assert f"Current Farming address: {address}, with a balance of: 21000000.0 THDD." in result.output  # default

    assert (
        f"Address: {address} has a balance of: 21000000000000000000 byte, with a total of: 2 transactions."
        in result.output
    )  # -ia
    assert "Coin Amount: 2625000000000000000 byte" in result.output  # -ic
