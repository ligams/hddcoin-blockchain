...
from hddcoin.wallet.util.tx_config import DEFAULT_TX_CONFIG, TXConfig
...
def test_get_transaction(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Status: In mempool",
        "Amount sent: 0.000012345678 HDD",
...
def test_get_transactions(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Amount received: 0.00001234568 HDD",
        "Amount received in clawback as sender: 0.000012345681 HDD",
...
def test_sign_message(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    inst_rpc_client = TestWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    hdd_addr = encode_puzzle_hash(get_bytes32(1), "xch")
    ...
    run_cli_command_and_assert(capsys, root_dir, command_args + [f"-a{hdd_addr}"], assert_list)
    expected_calls: logType = {
        "sign_message_by_address": [(hdd_addr, message.hex())],  # xch std
    }
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
    ...
    expected_calls: logType = {
        ...
        "nft_calculate_royalties": [
            (
                ...
                {"HDD": 10000000000000, "test3": 100000},
            )
        ],
        "create_offer_for_ids": [
            (
...
def test_take_offer(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        "  OFFERED:\n"
        "    - HDD (Wallet ID: 1): 10.0 (10000000000000 mojos)\n"
