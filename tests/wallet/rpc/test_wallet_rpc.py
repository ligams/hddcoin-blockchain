...
from hddcoin.util.streamable import ConversionError, InvalidTypeError
...
from hddcoin.wallet.util.query_filter import AmountFilter, HashFilter, TransactionTypeFilter
...
async def test_send_transaction(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(await wallet_2.get_new_puzzlehash(), "thdd")
...
async def test_spend_clawback_coins(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await wallet_1_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_2_puzhash, "thdd"),
        fee=uint64(0),
    ...
    tx = await wallet_2_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_1_puzhash, "thdd"),
        fee=uint64(0),
        puzzle_decorator_override=[{"decorator": "CLAWBACK", "clawback_timelock": 5}],
    )
    assert tx.spend_bundle is not None
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(await wallet.get_new_puzzlehash(), "thdd")
    )  # Create a pending tx
    ...
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "thdd"))
    await client.farm_block(encode_puzzle_hash(ph_by_addr, "thdd"))
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    tx_for_address = await client.get_transactions(1, to_address=encode_puzzle_hash(ph_by_addr, "thdd"))
...
async def test_cat_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    # Creates a CAT wallet with 100 bytes and a CAT with 20 bytes
...
async def test_offer_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    # Creates a CAT wallet with 20 bytes
    ...
    # Create an offer of 5 hddcoin for one CAT
    ...
    assert id == offer.name()
    assert summary == {"offered": {"hdd": 5}, "requested": {cat_asset_id.hex(): 1}, "infos": driver_dict, "fees": 1}
...
async def test_nft_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    res = await wallet_1_rpc.mint_nft(
        ...
        ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"],
    )
    assert res["success"]
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(ph, "thdd")
    tx_amount = uint64(15600000)
...
# $ hddcoin keys sign -d 'hello world' -t 'm/12381/8444/1/1'
#
# Example 2:
# $ hddcoin wallet get_address
# hdd1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ hddcoin wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a hdd1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        # Valid signatures
        (
            # hddcoin keys sign -d "Let's eat, Grandma" -t "m/12381/8444/1/1"
            {
        ...
        (
            # hddcoin wallet sign_message -m $(echo -n 'Happy happy joy joy' | xxd -p)
            # -a hdd1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a hdd1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "hdd1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh",
            },
            {"isValid": True},
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "hdd1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsjen7t0",
            },
...
async def test_cat_spend_run_tail(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await client.cat_spend(
        cat_wallet_id,
        amount=uint64(0),
        inner_address=encode_puzzle_hash(our_ph, "thdd"),
