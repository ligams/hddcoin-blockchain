...
async def test_nft_mint_from_did(
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"]), ("h", bytes32.random(seeded_random).hex())]
            ),
...
async def test_nft_mint_from_did_rpc(
    ...
    try:
        ...
        hdd_coins = await client.select_coins(
...
async def test_nft_mint_from_did_multiple_xch(
    ...
    hdd_coins_1 = await wallet_maker.select_coins(amount=10000, coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG)
    hdd_coins_2 = await wallet_maker.select_coins(
        amount=10000,
        coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG.override(excluded_coin_ids=[c.name() for c in hdd_coins_1]),
    )
...
@pytest.mark.anyio
async def test_nft_mint_from_hdd_multiple_xch(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
