...
from hddcoin.simulator.time_out_assert import adjusted_timeout, time_out_assert, time_out_assert_not_none
    ...
    tr1 = await api_0.nft_mint_nft(
        {
            ...
            "uris": ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"],
        }
    )
...
async def test_nft_wallet_rpc_update_metadata(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    uris = coin["data_uris"]
    assert len(uris) == 1
    assert "https://www.hddcoin.org/img/branding/hddcoin-logo.svg" in uris
...
async def test_nft_with_did_wallet_creation(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"],
            "target_address": encode_puzzle_hash(nft_ph, "thdd"),
        }
    )
    assert resp.get("success")
    sb = resp["spend_bundle"]
    ...
    assert did_nft["data_uris"][0] == "https://www.hddcoin.org/img/branding/hddcoin-logo.svg"
...
async def test_nft_transfer_nft_with_did(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    resp = await api_0.nft_transfer_nft(
        dict(
            wallet_id=nft_wallet_0_id,
            target_address=encode_puzzle_hash(ph1, "hdd"),
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"],
            "mu": ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"],
            "did": hex_did_id,
        }
    )
    assert resp.get("success")
...
async def test_nft_bulk_transfer(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    ph = await wallet_0.get_new_puzzlehash()
    ph1 = await wallet_1.get_new_puzzlehash()
    address = encode_puzzle_hash(ph1, AddressType.HDD.hrp(wallet_node_1.config))
    if trusted:
        wallet_node_0.config["trusted_peers"] = {
            full_node_api.full_node.server.node_id.hex(): full_node_api.full_node.server.node_id.hex()
        }
