...
from hddcoin.wallet.outer_puzzles import create_asset_id, match_puzzle
...
async def test_nft_offer_sell_nft(
    ...
    offer_did_nft_for_xch = {nft_to_offer_asset_id: -1, wallet_maker.id(): hdd_requested}
    ...
    expected_royalty = uint64(hdd_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + hdd_requested + expected_royalty
    expected_taker_balance = funds - taker_fee - hdd_requested - expected_royalty
...
async def test_nft_offer_request_nft(
    ...
    offer_dict = {nft_to_request_asset_id: 1, wallet_maker.id(): -hdd_offered}
    ...
    # assert payments and royalties
    expected_royalty = uint64(hdd_offered * royalty_basis_pts / 10000)
    expected_maker_balance = funds - maker_fee - hdd_offered - expected_royalty
    expected_taker_balance = funds - 2 - taker_fee + hdd_offered + expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_did_to_did(
    ...
    expected_royalty = uint64(hdd_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + hdd_requested + expected_royalty
    expected_taker_balance = funds - 1 - taker_fee - hdd_requested - expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_complex_nft_offer(
    ...
    """
    This test is going to create an offer where the maker offers 1 NFT and 1 CAT for 2 NFTs, an HDD and a CAT
    """
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    ...
    maker_hdd_royalties_expected = maker_royalty_summary[nft_to_offer_asset_id_maker][0]["amount"]
    ...
    funds_maker = int(funds_maker - FEE + HDD_REQUESTED + maker_hdd_royalties_expected)
    funds_taker = int(funds_taker - FEE - HDD_REQUESTED - maker_hdd_royalties_expected)
    ...
    # Now let's make sure the final wallet state is correct
    funds_maker = int(funds_maker + HDD_REQUESTED / 2)
    funds_taker = int(funds_taker - HDD_REQUESTED / 2)
