...
async def test_nft_offer_with_fee(
    ...
    hdd_request = 100
    maker_fee = uint64(10)
    offer_nft_for_xch = {wallet_maker.id(): hdd_request, nft_asset_id: -1}
    maker_unused_index = (
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + hdd_request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - hdd_request - taker_fee)
    ...
    hdd_offered = 1000
    maker_fee = uint64(10)
    offer_hdd_for_nft = {wallet_maker.id(): -hdd_offered, nft_to_buy_asset_id: 1}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_hdd_for_nft, tx_config, driver_dict_to_buy, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
    ...
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - hdd_offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + hdd_offered - taker_fee)
...
async def test_nft_offer_nft_for_nft(
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.hddcoin.org/image2.html"]),
