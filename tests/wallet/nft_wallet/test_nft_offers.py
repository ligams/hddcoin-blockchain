...
async def test_nft_offer_with_fee(
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 hdd
    ...
    hdd_request = 100
    maker_fee = uint64(10)
    offer_nft_for_hdd = {wallet_maker.id(): hdd_request, nft_asset_id: -1}
    maker_unused_index = (
    ...
    if forwards_compat:
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_nft_for_hdd, driver_dict, fee=maker_fee, reuse_puzhash=reuse_puzhash
        )
    ...
    # MAKE SECOND TRADE: 100 hdd for 1 NFT
    ...
    if forwards_compat:
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_hdd_for_nft, driver_dict_to_buy, fee=maker_fee
        )
        assert success is True
        assert error is None
        assert trade_make is not None
...
async def test_nft_offer_cancellations(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_hdd, driver_dict, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
