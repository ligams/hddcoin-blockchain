...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/HDD coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, cost_logger):
        async with sim_and_client() as (sim, sim_client):
            ...
            hddcoin_coins: List[Coin] = all_coins[None]
            ...
            # Create an HDD Offer for RED
            hddcoin_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            hddcoin_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                hddcoin_requested_payments, hddcoin_coins
            )
            hddcoin_announcements: List[Announcement] = Offer.calculate_announcements(hddcoin_requested_payments, driver_dict)
            hddcoin_secured_bundle: SpendBundle = generate_secure_bundle(hddcoin_coins, hddcoin_announcements, 1000)
            hddcoin_offer = Offer(hddcoin_requested_payments, hddcoin_secured_bundle, driver_dict)
            assert not hddcoin_offer.is_valid()

            # Create a RED Offer for HDD
            ...
            new_offer = Offer.aggregate([hddcoin_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for HDD and RED
