...
async def test_cat_trades(
    ...
    wallet_maker = env_maker.hdd_wallet
    wallet_taker = env_taker.hdd_wallet
    ...
    hddcoin_for_multiple_cat: OfferSummary = {
        wallet_maker.id(): -7,
        cat_wallet_maker.id(): 8,
        new_cat_wallet_maker.id(): 9,
    }
    multiple_cat_for_hddcoin: OfferSummary = {
        wallet_maker.id(): 10,
        cat_wallet_maker.id(): -11,
        new_cat_wallet_maker.id(): -12,
    }
    hddcoin_and_cat_for_cat: OfferSummary = {
        wallet_maker.id(): -13,
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        hddcoin_for_cat, wallet_environments.tx_config, fee=uint64(1)
    )
        ...
        [
            WalletStateTransition(
                ...
                post_block_balance_updates={
                    "xch": {
                        "pending_coin_removal_count": -1,
                        "confirmed_wallet_balance": -2,  # One for offered HDD, one for fee
                        "unconfirmed_wallet_balance": -2,  # One for offered HDD, one for fee
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                    },
                    "new cat": {
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "xch": {
                        ...
                        "<=#spendable_balance": -2,
                        "<=#max_send_amount": -2,
                        # Unconfirmed balance doesn't change because receiveing 1 HDD and spending 1 in fee
                        "unconfirmed_wallet_balance": 0,
                    },
                    "new cat": {
                        "unconfirmed_wallet_balance": -2,
                ...
                post_block_balance_updates={
                    "xch": {
                        ...
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                        # Confirmed balance doesn't change because receiveing 1 HDD and spending 1 in fee
                        "confirmed_wallet_balance": 0,
                    },
                    "new cat": {
                        "confirmed_wallet_balance": -2,
    ...
    # cat_for_hddcoin
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        cat_for_hddcoin, wallet_environments.tx_config
    )
    assert error is None
    assert success is True
    assert trade_make is not None
    ...
    hdd_tx: TransactionRecord = next(tx for tx in tx_records if tx.wallet_id == 1)
    assert hdd_tx.amount == 3
    assert hdd_tx.fee_amount == 1
...
class TestCATTrades:
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, hdd_to_cat_amount)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, hdd_to_cat_amount)
        maker_funds -= hdd_to_cat_amount
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_hddcoin, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(hddcoin_for_cat, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_aggregated_trade_state(self, wallets_prefarm):
        ...
        success, trade_make_1, error = await trade_manager_maker.create_offer_for_ids(hddcoin_for_cat, DEFAULT_TX_CONFIG)
        ...
        success, trade_make_2, error = await trade_manager_maker.create_offer_for_ids(cat_for_hddcoin, DEFAULT_TX_CONFIG)
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, hdd_to_cat_amount - 1)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, hdd_to_cat_amount - 1)
