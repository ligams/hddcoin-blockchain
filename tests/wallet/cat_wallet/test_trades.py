...
class TestCATTrades:
    ...
    async def test_cat_trades(
        ...
        MAKER_HDDCOIN_BALANCE = initial_maker_balance - 100
        TAKER_HDDCOIN_BALANCE = initial_taker_balance - 100
        await time_out_assert(25, wallet_maker.get_confirmed_balance, MAKER_HDDCOIN_BALANCE)
        await time_out_assert(25, wallet_taker.get_unconfirmed_balance, TAKER_HDDCOIN_BALANCE)
        ...
        if forwards_compat:
            ...
            success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
                hddcoin_for_cat, fee=uint64(1), reuse_puzhash=reuse_puzhash
            )
            await asyncio.sleep(1)
        ...
        MAKER_HDDCOIN_BALANCE -= 2  # -1 and -1 for fee
        MAKER_NEW_CAT_BALANCE += 2
        TAKER_HDDCOIN_BALANCE += 0  # +1 and -1 for fee
        TAKER_NEW_CAT_BALANCE -= 2

        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_HDDCOIN_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_HDDCOIN_BALANCE)
        await time_out_assert(15, wallet_maker.get_unconfirmed_balance, MAKER_HDDCOIN_BALANCE)
        ...
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_HDDCOIN_BALANCE)
        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_HDDCOIN_BALANCE)
        ...
        if forwards_compat:
            ...
            success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_hddcoin)
            assert error is None
            assert success is True
            assert trade_make is not None
        ...
        MAKER_CAT_BALANCE -= 4
        MAKER_HDDCOIN_BALANCE += 3
        TAKER_CAT_BALANCE += 4
        TAKER_HDDCOIN_BALANCE -= 3
        ...
        MAKER_HDDCOIN_BALANCE -= 7
        MAKER_CAT_BALANCE += 8
        MAKER_NEW_CAT_BALANCE += 9
        TAKER_HDDCOIN_BALANCE += 7
        TAKER_CAT_BALANCE -= 8
        ...
        MAKER_HDDCOIN_BALANCE += 10
        TAKER_CAT_BALANCE += 11
        TAKER_NEW_CAT_BALANCE += 12
        TAKER_HDDCOIN_BALANCE -= 10
        ...
        MAKER_HDDCOIN_BALANCE -= 13
        MAKER_CAT_BALANCE -= 14
        MAKER_NEW_CAT_BALANCE += 15
        TAKER_HDDCOIN_BALANCE += 13
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        async with wallet_node_maker.wallet_state_manager.lock:
            cat_wallet_maker: CATWallet = await CATWallet.create_new_cat_wallet(
                wallet_node_maker.wallet_state_manager, wallet_maker, {"identifier": "genesis_by_id"}, hdd_to_cat_amount
            )
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(hddcoin_for_cat)
        assert error is None
        assert success is True
        assert trade_make is not None
