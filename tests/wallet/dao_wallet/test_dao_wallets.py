...
from hddcoin.wallet.dao_wallet.dao_info import DAORules
...
from hddcoin.wallet.util.tx_config import DEFAULT_TX_CONFIG
...
async def test_dao_creation(
    ...
    assert e_info.value.args[0] == f"Your balance of {funds} bytes is not enough to create {funds + 1} CATs"
...
async def test_dao_funding(
    ...
    hdd_funds = uint64(500000)
    cat_funds = uint64(100000)
    funding_tx = await dao_wallet_0.create_add_funds_to_treasury_spend(hdd_funds, DEFAULT_TX_CONFIG)
    ...
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, hdd_funds)
    ...
    hdd_proposal_inner = generate_simple_proposal_innerpuz(
        treasury_id,
        [recipient_puzzle_hash],
        [proposal_amount_1],
        [None],
    )
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal)
    ...
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, hdd_funds - 10000)
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, cat_funds, cat_id)
    await time_out_assert(30, dao_wallet_1.get_balance_by_asset_type, hdd_funds - 10000)
...
async def test_dao_proposals(
    ...
    # Create funding spend so the treasury holds some HDD
    hdd_funds = uint64(500000)
    funding_tx = await dao_wallet_0.create_add_funds_to_treasury_spend(hdd_funds, DEFAULT_TX_CONFIG)
    await wallet_0.wallet_state_manager.add_pending_transaction(funding_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[funding_tx], timeout=60)
    ...
    await time_out_assert(10, dao_wallet_0.get_balance_by_asset_type, hdd_funds)
    await time_out_assert(10, dao_wallet_1.get_balance_by_asset_type, hdd_funds)
    await time_out_assert(10, dao_wallet_2.get_balance_by_asset_type, hdd_funds)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        hdd_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, fee=base_fee
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    hdd_proposal_inner = Program.to(["x"])
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        hdd_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, fee=base_fee
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    ...
    await time_out_assert(
        20, dao_wallet_0.get_balance_by_asset_type, hdd_funds - proposal_amount_1 + proposal_min_amt - 1
    )
    ...
    await time_out_assert(20, wallet_2.get_confirmed_balance, funds + proposal_amount_1)
    expected_balance = hdd_funds - proposal_amount_1 + (3 * proposal_min_amt) - 3 - new_mint_amount
...
async def test_dao_rpc_api(
    ...
    hdd_funding_amt = 200000
    hdd_tx = await api_0.dao_add_funds_to_treasury(
        dict(
            wallet_id=dao_wallet_0_id,
            amount=hdd_funding_amt,
            funding_wallet_id=1,
        )
    )
    txs = [TransactionRecord.from_json_dict(cat_tx["tx"]), TransactionRecord.from_json_dict(hdd_tx["tx"])]
    await full_node_api.wait_transaction_records_entered_mempool(records=txs, timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)

    expected_xch -= hdd_funding_amt + new_cat_amt
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_xch)
    ...
    assert balances["balances"]["xch"] == hdd_funding_amt
...
async def test_dao_rpc_client(
    ...
    try:
        ...
        # fund treasury
        hdd_funds = uint64(10000000000)
        funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, hdd_funds, DEFAULT_TX_CONFIG)
        ...
        await rpc_state(20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["xch"], hdd_funds)
        ...
        await rpc_state(
            20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["xch"], hdd_funds - 1000
        )
        ...
        expected_xch = initial_funds - amount_of_cats - new_cat_amt - hdd_funds - (2 * fee) - 2 - 9000
        await rpc_state(
...
async def test_dao_complex_spends(
    ...
    try:
        ...
        hdd_funds = uint64(10000000000)
        for _ in range(4):
            funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, uint64(hdd_funds / 4), DEFAULT_TX_CONFIG)
            cat_funding_tx = await client_0.dao_add_funds_to_treasury(
        ...
        additions = [
            {"puzzle_hash": ph_0.hex(), "amount": hdd_funds / 4},
            {"puzzle_hash": ph_1.hex(), "amount": hdd_funds / 4},
        ]
        proposal = await client_0.dao_create_proposal(
            wallet_id=dao_id_0,
            proposal_type="spend",
...
async def test_dao_concurrency(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        hdd_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, uint64(1000)
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_reorgs(
    ...
    # Check that the funding spend is recognized by both dao wallets
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, hdd_funds)
    await time_out_assert(20, dao_wallet_1.get_balance_by_asset_type, hdd_funds)
...
async def test_dao_votes(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, vote_1, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, vote_2, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    vote_5 = uint64(1)
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, vote_5, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
...
async def test_dao_resync(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG, uint64(10))
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(hdd_proposal_inner, DEFAULT_TX_CONFIG)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
