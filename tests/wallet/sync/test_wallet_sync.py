...
from hddcoin.protocols.shared_protocol import Capability
from hddcoin.protocols.wallet_protocol import RequestAdditions, RespondAdditions, RespondBlockHeaders, SendTransaction
...
from hddcoin.wallet.util.wallet_sync_utils import PeerRequestException
...
from hddcoin.wallet.wallet_weight_proof_handler import get_wp_fork_point
...
class TestWalletSync:
    ...
    6. Clear all coins from the dust wallet.
       Send to the dust wallet "spam_filter_after_n_txs" coins that are equal in value to "hdd_spam_amount".
       Send 1 byte from the dust wallet. The dust wallet should receive a change coin valued at "hdd_spam_amount-1".
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "spam_filter_after_n_txs, hdd_spam_amount, dust_value",
        [
            ...
            (100, 1000000, 1),  # default filter level (1m bytes), default dust size (1)
        ],
    )
    async def test_dusted_wallet(
        ...
        # It's also possible to obtain the current settings for spam_filter_after_n_txs and hdd_spam_amount
        # spam_filter_after_n_txs = wallets[0][0].config["spam_filter_after_n_txs"]
        # hdd_spam_amount = wallets[0][0].config["hdd_spam_amount"]
        # dust_value=1
        ...
        # Current max value for hdd_spam_amount is 0.01 HDD.
        ...
        assert hdd_spam_amount >= 1
        assert hdd_spam_amount <= 10000000000
        assert dust_value >= 1
        ...
        # The dust is only filtered at this point if spam_filter_after_n_txs is 0 and hdd_spam_amount is > dust_value.
        if spam_filter_after_n_txs > 0:
            dust_coins = 1
            large_dust_coins = 0
            large_dust_balance = 0
        elif hdd_spam_amount <= dust_value:
            dust_coins = 0
            large_dust_coins = 1
        ...
        small_unspent_count = len([r for r in all_unspent if r.coin.amount < hdd_spam_amount])
        ...
        log.info(f"hdd_spam_amount {hdd_spam_amount}")
        ...
        for i in range(large_coins):
            payee_ph = await dust_wallet.get_new_puzzlehash()
            payees.append({"amount": uint64(hdd_spam_amount), "puzzlehash": payee_ph, "memos": []})
        ...
        large_coin_balance = large_coins * hdd_spam_amount

        # Determine whether the filter should have been activated.
        # Make sure the number of coins matches the expected number.
        # At this point, nothing should be getting filtered unless spam_filter_after_n_txs is 0.
        assert dust_coins == spam_filter_after_n_txs
        assert balance == dust_coins * dust_value + large_coins * hdd_spam_amount + large_dust_balance
        ...
        for i in range(5):
            ...
            payees.append({"amount": uint64(hdd_spam_amount + i), "puzzlehash": payee_ph, "memos": []})
            large_coins += 1
            large_coin_balance += hdd_spam_amount + i

            payee_ph = await dust_wallet.get_new_puzzlehash()

            # Make sure we are always creating coins with a positive value.
            if hdd_spam_amount - dust_value - i > 0:
                payees.append({"amount": uint64(hdd_spam_amount - dust_value - i), "puzzlehash": payee_ph, "memos": []})
            else:
                payees.append({"amount": uint64(dust_value), "puzzlehash": payee_ph, "memos": []})
            # In cases where hdd_spam_amount is sufficiently low,
            # the new dust should be considered a large coina and not be filtered.
            if hdd_spam_amount <= dust_value:
                large_dust_coins += 1
                large_dust_balance += dust_value
        ...
        # The size of the coin to send the dust wallet is the same as hdd_spam_amount
        if hdd_spam_amount > 1:
            coin_value = hdd_spam_amount
        else:
            # Handle the edge case to make sure the coin is at least 2 bytes
        ...
        # Send a 1 byte coin from the dust wallet to the farm wallet
        payee_ph = await farm_wallet.get_new_puzzlehash()
        ...
        # Make sure the dust wallet received a change coin worth 1 byte less than the original coin size
        if spam_filter_after_n_txs > 0:
            assert unspent_count == spam_filter_after_n_txs
        else:
            # in the edge case there should be 1 coin
        ...
        metadata = Program.to(
            [
                ("u", ["https://www.hddcoin.org/img/branding/hddcoin-logo.svg"]),
