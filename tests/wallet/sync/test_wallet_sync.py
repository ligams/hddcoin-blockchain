...
class TestWalletSync:
    ...
    async def test_dusted_wallet(
        ...
        for i in range(large_coins):
            payee_ph = await dust_wallet.get_new_puzzlehash()
            payees.append(Payment(payee_ph, uint64(hdd_spam_amount)))
        ...
        for i in range(5):
            ...
            payees.append(Payment(payee_ph, uint64(hdd_spam_amount + i)))
            large_coins += 1
            ...
            if hdd_spam_amount - dust_value - i > 0:
                payees.append(Payment(payee_ph, uint64(hdd_spam_amount - dust_value - i)))
            else:
                payees.append(Payment(payee_ph, uint64(dust_value)))
