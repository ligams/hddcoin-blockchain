...
from hddcoin.wallet.util.address_type import AddressType, ensure_valid_address, is_valid_address
...
def test_is_valid_address_did(config_with_address_prefix: Dict[str, Any]) -> None:
    config = config_with_address_prefix
    valid = is_valid_address(
        "did:hddcoin:14jxdtqcyp3gk8ka0678eq8mmtnktgpmp2vuqq3vtsl2e5qr7fyrsr9gsr7",
...
def test_is_valid_address_did_bad_address(config_with_address_prefix: Dict[str, Any]) -> None:
    config = config_with_address_prefix
    valid = is_valid_address(
        "did:hddcoin:14jxdtqcyp3gk8ka0678eq8mmtnktgpmp2vuqq3vtsl2e5qr7fyrsrxxxxx",
        allowed_types={AddressType.DID},
        config=config,
    )
    assert valid is False
...
@pytest.mark.parametrize("prefix", [None])
def test_ensure_valid_address_hdd_bad_address(config_with_address_prefix: Dict[str, Any]) -> None:
    config = config_with_address_prefix
...
def test_ensure_valid_address_did(config_with_address_prefix: Dict[str, Any]) -> None:
    ...
    assert address == "did:hddcoin:14jxdtqcyp3gk8ka0678eq8mmtnktgpmp2vuqq3vtsl2e5qr7fyrsr9gsr7"
