...
from hddcoin.wallet.util.compute_hints import HintedCoin
...
class _AssertRuntime:
    ...
    Produces output of the following form.
        ...
        /home/altendky/repos/hddcoin-blockchain/tests/core/full_node/test_performance.py:187
    ...
    def __exit__(
        ...
        if self.record_property is not None:
            ...
            relative_path_str = (
                pathlib.Path(results.entry_file).relative_to(pathlib.Path(hddcoin.__file__).parent.parent).as_posix()
            )
...
@contextlib.contextmanager
def closing_hddcoin_root_popen(hddcoin_root: ChiaRoot, args: List[str]) -> Iterator[subprocess.Popen[Any]]:
    environment = {**os.environ, "CHIA_ROOT": os.fspath(hddcoin_root.path)}
